...
виртуализация гридов (объёмы малы), live-курсоры партнёров (шум/трафик; пинги II.12.8
покрывают потребность), rebinding-UI (шов через карты уже есть). Пинги — НЕ отвергнуты,
см. II.12.8 (шов обязателен).
