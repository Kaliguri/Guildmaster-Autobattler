...
Отвергнуто осознанно: текст/войс-чат кооперативa (при дизайне коопа; Steam Voice —
кандидат), аналитика UI-кликов (отдельная инфраструктура), photosensitivity-настройки
(частично покрыты тумблерами анимаций; остальное — домен боевого VFX).
