...
> Исполняемое техзадание Фазы 4 «Контент-каркас». **Дизайн-источник — [[tech/reference/data-layer|Reference - Data Layer]] (ред. 3)** — при любой неоднозначности этого ТЗ решает файл 13; при конфликте файла 13 с реальным кодом — спросить Макса, не угадывать.
...
4. **`StatsConfig`:** заполнить `Defaults` базовыми значениями юнита (согласовать список с Максом по факту: MaxHP, MoveSpeed, AttackRange, AttackSpeed…— свериться с [[tech/planning/stat-system|Planning - Stat System]] и текущими значениями болванчика/реликвий, чтобы дефолты не изменили баланс семёрки).
