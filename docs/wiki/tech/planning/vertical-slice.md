...
**`RunState` (durable, по строковым ID) = единственный источник истины забега; `GameFlow` (async UniTask) = оркестратор; узлы карты = полиморфные `IEventFlow`; бой/награда/сцена — эфемерные слои, собираемые из `RunState`.** Ровно модель [[tech/explanation/run-flow|Explanation - Run Flow]] §2–4.
...
Data-first, как в [[tech/planning/deployment-encounters|Planning - Deployment & Encounters]]. Каждый шаг оставляет играбельный результат.

### Часть A — скелет петли (замыкает «бой → исход → награда → следующий бой»)

**Шаг A1 — `RunState` + DTO + автосейв + `GameConfig`-лимиты** `[фундамент]`
- `RunState` POCO (§3.1) в `Guildmaster.Guild` (или `Core`), DTO-слой + ES3-плумбинг ([[data-layer-principles]]: SO→POCO→DTO). Три точки автосейва ([[tech/explanation/run-flow|Explanation - Run Flow]] §5).
...
- `BattleFlow`: Prep (готово — `DeploymentController`+loadout) → Combat (готово) → **Outcome** (новое): по `BattleOutcome` показать исход, ретраи (до 2, [[tech/explanation/run-flow|Explanation - Run Flow]] §6), вернуть `EventResult`.
...
- `MapNodeData`/граф (§3.3), генерация из сида ИЛИ загрузка `RunTemplateData`. Оверлей-UI (read-only поверх сцены, [[tech/explanation/run-flow|Explanation - Run Flow]] §9) для выбора следующего узла. `MapState` в `RunState`.
