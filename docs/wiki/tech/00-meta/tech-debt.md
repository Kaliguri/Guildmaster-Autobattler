...
**1. `UiPreviewCatalog` пережил свою задачу и теперь ВРЁТ.** Старая витрина показывает 7 позиций из
14 контролов, а вкладку собирает из голого `Button` с классом `gm-tab` — тогда как экраны используют
`gm:PlateButton`. Все состояния активной вкладки висят на `--gm-plate-*`, которых обычная кнопка не
читает: в витрине они не работают, и это читается как «состояние не реализовано». С появлением
контактного листа (`Alebardium → UI → Contact Sheet`) витрина дублирует его задачу и делает это
неверно. **Чинить так:** снести и перенаправить `AGENTS.md` на лист; либо, если она нужна для
чего-то ещё, — строить образцы через `UiSampleFactory`, чтобы владелец сборки был один.
...
### 5.9 Типографика привязана к экранам так же, как были кнопки (06.08.2026)

Кнопки сведены к двум ролям, текст описан восемью — но САМИ КЛАССЫ не сведены. В теме **75 классов**
задают гарнитуру, кегль или разрядку, и почти все привязаны к экрану, а не к роли. Роль «тело
текста» размазана по четырём: `gm-tooltip__desc`, `gm-detail__desc`, `gm-event-body`,
`gm-loadout__narrative-text`. Витрина показывает по одному ПРЕДСТАВИТЕЛЮ на роль — то есть отвечает
на вопрос «какой текст брать», но не мешает завести семьдесят шестой класс.

**Чинить так:** тем же приёмом, каким сведены кнопки. Разобрать каждый класс на «это роль» или «это
раскладка/размер»: роль уезжает в `components/text.uss` под именем роли, раскладка — на контейнер
экрана. Признак раскладки тот же — класс задаёт только отступы, ширину или выравнивание. Гейтом
может стать правило «класс с `font-size` вне `components/` — дефект», но заводить его до сведения
бессмысленно: он будет красным на семидесяти пяти строках.

Связанный хвост: карточки и строки живут той же болезнью — `gm-shop__card` рядом с `gm-card`,
`gm-shop__stash-row` рядом с `gm-reward-drop__row` (см. §5.8 п.2). Приём отработан на кнопках,
осталось применить.
