...
**Чинится сигналом «бой начался заново» по каналу `BattleControl`** — хост шлёт его там же, где
зовёт `TapeStreamer.Reset`, гость на приёме сбрасывает обе свои половины. Заодно тем же сигналом
поедет фаза боя, которой у гостя сейчас нет (боевой UI у него молчит). Отдельный заход, не срезка в
чужом: канал уже есть, и правило «одно сообщение — один смысл» тут важнее экономии.
...
### 3.23 `IBattleAuthority` спрашивают каждый кадр, хотя роль уже известна на входе
