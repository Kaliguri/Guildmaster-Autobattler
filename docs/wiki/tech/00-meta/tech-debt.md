...
### 3.0 КРАСНЫЙ ТЕСТ: зеркало расходится на хиле Друида (открыто 2026-07-30)

`MirrorMatchTests.Mirror_SquadSeries_NeverDiverges(5)` падает **детерминированно**: отряд
[Defender, Dreameater, Druid, Emberkeeper], тик 543 (18.1 с), у левого Defender висит
`effect.spore_mend`, у правого нет. Все прочие числа пары совпадают до знака — HP, щит, позиция,
фаза, кулдаун. То есть Друид одной стороны вылечил защитника, а зеркальный — нет.

Это **не про типы урона** (реформа 2026-07-30): у `spore_burst` множитель урона нулевой, тип там
вообще не участвует, а данные отряда после миграции эквивалентны прежним. Расхождение живёт в выборе
цели — то есть в том же классе дефекта, что уже записан по `ProfileBrain` и `MarkTransferComponent`:
**несимметричный тай-брейк при равных значениях** (`o.Id < best.Id` симметрию зеркала не сохраняет).

Появилось в дереве, где уже работал **«удар с разбега»** (12b56a928) — он меняет тайминги сближения и
замаха, а значит и то, в какой тик кто из Друидов дотягивается до каста. Проверить в первую очередь
его: прогнать тот же тест на состоянии до того коммита.

Инвариант зеркального детерминизма — HARD, поэтому пункт красный, а не «отложенный»: пока тест падает,
любой вывод балансного стенда о равных составах под вопросом.
