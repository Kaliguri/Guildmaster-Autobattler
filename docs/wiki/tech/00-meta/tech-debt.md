...
**Ещё восемь вопросов, которые серия трогает молча** (собраны при разборе, ждут того же захода):

1. **Два контакта в одном тике.** При 30 Гц тик = 0.033 с; близкие маркеры округлятся в один тик, и
   модель «урон одним коммитом» ([[tech/40-planning/tick-resolution|Planning - Tick Resolution]])
   подаст их как один удар. Нужен минимальный зазор в тиках или явное правило слияния.
2. **On-hit на каждом контакте.** `ApplyAutoAttackOnHit` висит на контакте — поджоги, метки, стаки
   льда Криоманта пойдут кратно числу ударов. Решить: каждый контакт / только первый / только последний.
3. **Что считается «первой атакой»** для заявки о скейлах первой атаки ([[gdd/20-combat/stats|Combat -
   Stats]] §На будущее) — первый контакт или весь свинг.
4. **Каст между контактами.** Сейчас `IsCastBusy` не рвёт уже занесённый замах (M18); при серии надо
   сказать, может ли способность встрять в середину.
