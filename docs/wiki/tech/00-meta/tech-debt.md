...
### 5.1 Пять реализационных скиллов разошлись с кодом
`gamefeel-vfx` — критично, дальше `combat-sim`, `data-authoring`, `audio`, `uitk`. Скиллы
**предписывают**, а не описывают, поэтому заморозка справочника их не касается: расхождение с кодом
здесь настоящий дефект — агент получает инструкцию, которой код не соответствует. Снято аудитом
2026-07-26 (fan-out 13 агентов, снимок на `1a55d09d`), правки отложены Максом до рефактора.
**Чинить сверкой с живым кодом**, скилл за скиллом.
