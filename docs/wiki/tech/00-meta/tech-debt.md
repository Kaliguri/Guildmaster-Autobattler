...
| Реакция на входящий удар | `BlockComponent` (переименован из `BulwarkComponent` 2026-07-31, `[MovedFrom]`) и `DodgeComponent` — два самостоятельных `IPreDamageComponent` с зарядами, ICD, фильтром прямого попадания и гейтом дееспособности | парирование делать переопределением Блока, а не третьей копией. Шов зарядов (`RuntimeEffect.ArmCharges` / `TryConsumeCharge`) переиспользуется как есть |
...
**Чего в движке нет совсем:** парирования и Маскировки. Ритм (`CombatIdle`), Атака из нескольких
Ударов, Блок как примитив и рекаст — сделаны 2026-07-30/31.

**Порядок входа на остаток:** парирование на общем pre-damage-шве → Маскировка. Вторая единственная
трогает AI, и её удобнее делать, когда ритм уже устоялся.
