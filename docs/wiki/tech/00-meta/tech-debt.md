...
**Владельцы правды в коде (проставлены 2026-07-30):** `<remarks>` у `ClipMarkers` — «читается только
первый маркер, дизайн требует нескольких, расширять начинать отсюда»; тест
`WindupAutoAttackTests.Stun_AfterHit_DoesNotRushNextAttack` держит инвариант «стан не ускоряет атаку»
(зелёный на текущем дереве через ветку `Recovery`; наивный рефанд при серии его уронит). Не хватает
теста на многомаркерный клип — он появится вместе с самой серией.
...
|---|---|---|
| Рекаст авто-атаки | **три копии** обнуления таймера: `AbilitySystem` (умение-удар, M18), `VortexEntryComponent` (заход Монаха), `AutoAttackSystem.Interrupt` (рефанд) — плюс поля `NextWindupMult` и `EmpowerDamageMult`, которые ставятся врозь | один глагол на `RuntimeUnit`: «выйти вне очереди с такими-то множителями», по правилу «состояние за глаголами» |
| Реакция на входящий удар | `BulwarkComponent` и `DodgeComponent` — два самостоятельных `IPreDamageComponent` с зарядами, ICD, фильтром прямого попадания и гейтом дееспособности | общий Блок как примитив; «Оплот» и парирование — его переопределения. Шов зарядов (`RuntimeEffect.ArmCharges` / `TryConsumeCharge`) переиспользуется как есть |
| Микро-стан | поле `_microStun` **внутри** пассива Монаха: владельцев столько же, сколько носителей | один ассет-эффект, на который ссылаются и Монах, и парирование |
| Счёт «каждая N-я атака» | `EveryNthAttackComponent` считает по событию урона, то есть по **Ударам** | с введением серии обязан считать Атаки, иначе Драугр начнёт триггерить втрое чаще и молча |
| Видимость юнита | нет вовсе; `StealthComponent` отвечает только за то, **когда** кит уходит в тень | фильтр в единственной точке выбора цели (`ProfileBrain.SelectBest`) — она одна, и это редкая удача |

**Чего в движке нет совсем:** четвёртого состояния цикла атаки (`CombatIdle` — решение 2026-07-30/10,
кода нет; на нём стоит вопрос «кого ловит парирование»), Атаки из нескольких Ударов (см. §3.9),
парирования, Маскировки.

**Порядок входа, если брать тему целиком:** ритм (`CombatIdle` + Удар/Атака) → рекаст как глагол →
блок и парирование на общем pre-damage-шве → Маскировка. Первые три — тайминг и один шов; Маскировка
единственная трогает AI, и её удобнее делать, когда ритм уже устоялся.
