...
> Граф пересмотрен в рамках Фазы 1 (см. [10. Архитектура реализации — Фаза 1](implementation/02.%20Архитектура%20реализации%20—%20Фаза%201%20(Боевое%20ядро).md) §2). Старый набор (Core/Units/Combat/Guild/UI) был временным; здесь — финальная структура «как надо».
