...
**Детерминизм — два разных рандома, и они не смешиваются.**

- **Сим-рандом** — `IRngService` (§4). Забег, бой, карта, награды, магазин. Детерминирован между
  машинами; нарушение ломает кооп и воспроизведение по сиду.
- **Показ-рандом** — вариация формы удара, разброс осколков, зерно фактуры. Сходиться ему не с чем,
  поэтому `sin`-хеш прямо в шейдере здесь безопасен и дешевле проброса числа из C#.

Проверка, к какой стороне относится число: **читают ли его обе стороны шва?** Да — это сим-рандом
по определению, даже если выглядит как визуальная мелочь. Обратное течение — показ-рандом, попавший
в симуляцию, — инвариант из §1 и дефект.

**Общие правила для всего, что считается:**

- **Формула живёт в `Guildmaster.Procedural`, а не в файле-потребителе.** Шум, хеши и SDF-примитивы
  писались у нас заново трижды — в симуляции, в шейдере осколков и в запекателе панелей. Четвёртого
  раза быть не должно: разные шумы дают разную фактуру там, где задумана одна.
...
- **Запечённое детерминировано по сиду:** тот же сид и те же параметры дают побайтово тот же файл.
  Без этого каждый перепёк даёт визуальный диф на ровном месте, и ассет невозможно ревьюить.
