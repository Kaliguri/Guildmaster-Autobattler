...
>
> Связано: [[tech/10-reference/combat-model|Reference - Combat Model]], [[tech/40-planning/phase-1-combat-core|Planning - Phase 1: Combat Core]], слой презентации (код `Assets/_Project/Scripts/Presentation/`), [[tech/00-meta/tech-changelog|Meta - Tech Changelog & Decisions]], [[tech/20-explanation/run-flow|Explanation - Run Flow]].
...
**Разделение ответственности:** арена задаёт **зоны** (области). Энкаунтер (`EncounterData`, см. [[tech/20-explanation/run-flow|Explanation - Run Flow]]) задаёт, **кто и где** из врагов стоит внутри enemy-зоны. Player-зоны — куда игрок таскает своих.
...
- **Владелец `EncounterData`** и связь «энкаунтер -> ключ арены» — уточнить в [[tech/20-explanation/run-flow|Explanation - Run Flow]].
