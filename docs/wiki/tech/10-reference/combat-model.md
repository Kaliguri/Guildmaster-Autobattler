...
>
> **Вердикт получен (2026-07-26, журнал «ночь-3»): `AffinityTable` подлежит СНЯТИЮ.** Сродство
> несёт идентичность своим глаголом (яд — DoT+дебафф, свет — очищение+хил, тьма — голая мощь),
> а не множителями по типу цели; универсальная матрица «тип → уязвимость» была отклонена ещё
> 2026-07-15/35. Уязвимости вернутся позже **контентом**, как свойства конкретных врагов.
> **Раздел переписывать после правки кода**, не раньше: сейчас он верно описывает фактическое
> поведение. Задача — [[gdd/00-meta/roadmap|Meta - Roadmap]] §Долги кода.
