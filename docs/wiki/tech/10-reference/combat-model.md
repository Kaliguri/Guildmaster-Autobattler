...
**Статус:** Ready — §6 (две оси урона: школа vs сродство) дополнен 2026-07-16

---

> Архитектура боевого юнита: композиция «Сосуд + Реликвия», стат-система и единая модель эффектов.
> Опирается на принципы из [[tech/10-reference/tech-stack|5. Технологический стек и архитектура]]
...
| **«Сосуд»** | «пилот» | **Перки** — модификаторы поверх чемпиона (`PerkModifiers`). Сам статов не имеет. Супер-уник-слой «**Судьба**» — см. [GDD: 5. Реликвии](../../gdd/40-content/relics-overview.md) |
| **Гильдмастер** | «тренер на бровке» | Не юнит на поле и **не действует в бою**. Даёт **гильдие-широкие** модификаторы и задаёт стартовый набор забега |
...
- **Редкость ≠ множитель статов.** Влияние на статы примерно равно у любой редкости; редкость = **размер кита**: Треш (минимум, 0–1 способность) → Обычная (≈1 пассивка + 1 активка) → Уникальная (расширенный кит именных героев). Прежняя градация Common/Rare/Epic отменена — см. [GDD 5. Реликвии](../../gdd/40-content/relics-overview.md) и [GDD 12. Мета-прогрессия](../../gdd/30-run-meta/meta-progression.md).
- Ресурс привязан к Реликвии (сменил Реликвию → сменился тип ресурса).
- Гильдмастер: **только** гильдие-широкий `IModifierSource` на всех юнитов (+ стартовый набор забега). Рантайм-кастера нет: заклинания ГД **вырезаны** (2026-07), в бою **нет источников способностей вне юнитов** — см. [GDD 4. Гильдмастер](../../gdd/10-vision/guildmaster.md).
...
> 📋 **Полный канонический список статов (ровно 30, EN+RU, операции, дефолты, клампы)** — в отдельном доке [[tech/40-planning/stat-system|11. Стат-система]]. Ниже — только механика стат-системы; конкретные `StatType` см. там.
...
> Полная таблица семей множителей — [[tech/40-planning/stat-system|11. Стат-система §5]].
...
Урон — событие, проходящее **детерминированный конвейер БЕЗ RNG** (крита и уклонения нет — решение зафиксировано). Полная формула митигации (броня = %-формула `K/(K+armor)`, пробивание = плоское + процентное) — в [[tech/40-planning/stat-system|11. Стат-система §4]]. Порядок (`DamagePipeline`):
```
raw → ×Source.DamageDealtEff
    → броня ПО ШКОЛЕ (Physical→PhysArmor, Magical→MagicArmor;
                       пробивание плоское+%; K/(K+effArmor)); True — пропуск
    → ×AffinityTable(сродство, тип существа цели) — бронёй НЕ гасится, действует и на True
...
### 6.1 Две оси урона: школа vs сродство

> [!info] Граница вик (проставлено 2026-07-26 заходом по ГДД, содержание раздела не менялось)
> **Канон-дом самой модели урона — ГДД** ([[gdd/20-combat/stats|Combat - Stats]] §Школа vs сродство).
> Здесь — её **реализация**: имена enum, где живут override, как собирается `DamageType`.
>
> [!warning] `AffinityTable` СНЯТА ИЗ КОДА 2026-07-26 — раздел ниже устарел
> Класса `AffinityTable` больше нет, множителя в `DamagePipeline` нет: **сродство на число урона
> не влияет вообще**. Оно несёт идентичность своим глаголом (яд — DoT+дебафф, свет —
> очищение+хил, тьма — голая мощь); универсальная матрица «тип → уязвимость» была отклонена ещё
> 2026-07-15/35 и подтверждённо снята журналом «ночь-3». Уязвимости вернутся позже **контентом**,
> как свойства конкретных врагов. Поведение охраняет guard-тест
> `Affinity_NeverScalesDamage_ByCreatureType`. Тело раздела (формула §2.5 и список правил ниже)
> **описывает уже несуществующий код** и ждёт переписывания — план в `docs/tech-docs-sync-plan.md`.

Урон несёт **две ортогональные оси** (задаются на каждую атаку/способность; override `Inherit` = взять у юнита-кастера). ГДД «8» §«Школа vs сродство»:

- **Школа урона** (`DamageSchool`) — количественная ось, **гасится бронёй**. Намеренно мало школ: `Physical` (гасит `PhysArmor`), `Magical` (Огонь/Лёд/Молния/Аркана под ОДНОЙ магической бронёй `MagicArmor` — различия элементов живут в механике, не в резистах), `True` (идёт мимо брони).
- **Конкретика типа урона** — `PhysicalSubtype` (Blunt/Slash/Pierce) при физ-школе и `MagicElement` (Fire/Ice/Lightning/Arcane) при маг-школе. Задаются **поисточниково** (автоатака / способность через `*Override` / DoT-компонент); собираются в `DamageType`-струку (`DamageCategories.Resolve`). В пайплайне урона **не участвуют** — это read-model для тегов «быстрого чтения» (`UnitTagResolver`) и задел «влияют на урон позже».
- **Сродство урона** (`DamageAffinity`) — качественная ось, бронёй **НЕ гасится**, взаимодействует с типом существа цели. `None` / `Poison` / `Light` / `Dark`. Множитель ложится **поверх** школы, в том числе поверх `True`.
- **Тип существа** (`CreatureType`) — таксономия юнита (`Living` / `Undead` / `Construct` / `Demon` / `Beast`); есть и у реликвий, не только у врагов. Отдельно от фракции (фракция — организационная группа, тип существа — что юнит есть). Драйвит сродства.
- **Таблица сродств** (`AffinityTable`) — **статическая и детерминированная** (дизайн-правила, а не баланс-числа → никакого SO в тике): Яд иммунен (×0) на `Undead`/`Construct`; Свет уязвим (×1.3) по `Undead`/`Demon`; Тьма уязвима (×1.3) по `Living`; всё прочее нейтрально (×1).
...
Достраивает чек-лист детерминизма из [[tech/10-reference/tech-stack|5. Технологический стек и архитектура]].
...
`UnitSaveData` хранит `vesselId`, `equippedRelicId`, `perkIds[]`, `itemIds[]` — **строковые ID**. `RuntimeUnit` пересобирается из них + SO при загрузке, не сериализуется напрямую. См. [[tech/10-reference/saves|2. Сохранения]].

---

## 10. Движковые расширения под контент реликвий (бэклог)

> Фазы 1–2 дали движок эффектов/способностей и библиотеку компонентов (`StatModifier`, `PeriodicDamage/Heal`, `Control`, `Shield`, `Lifesteal`, `Thorns`, `Dispel`). Часть запланированных реликвий-классов (GDD [1. Контент - Реликвии](../../gdd/relics/index.md)) опирается на механики, которых в этой библиотеке **ещё нет**.
> **Решение по скоупу:** все 7 Обычных реликвий должны быть играбельны в бою к концу **Фазы 3** ([[tech/40-planning/roadmap|9. Roadmap §3]]), поэтому нужные им расширения затаскиваются в Фазу 3 (а не размазываются по Ф4).
...
- ~~Точный список `StatType` (~30)~~ — **решено**, см. [[tech/40-planning/stat-system|11. Стат-система]]. Дефолты в `StatsConfig` — баланс (Фаза 4+).
