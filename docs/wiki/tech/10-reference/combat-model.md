...
>
> **Расхождение по существу:** описанный ниже механизм «тип существа драйвит сродства» +
> `AffinityTable` — это универсальная матрица «тип → уязвимость», которую ГДД **отклонил**
> решением 2026-07-15/35 (тип существа оставлен «редкой специей» на отдельных врагах).
> В коде матрица работает (`DamagePipeline.cs`). Вынесено Максу на решение; до вердикта раздел
> **не переписывать** — он честно описывает то, что делает код.
