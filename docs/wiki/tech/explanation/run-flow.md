...
Генерится **целиком из сида на старте акта** (структура как Slay the Spire — игрок видит весь граф и планирует путь). Хранится в `RunState` (граф + позиция). Генерация карты, наград, пула магазина, ивентов — через `IRngService` (см. [[tech/planning/seed|3. Рандомизация]]).
