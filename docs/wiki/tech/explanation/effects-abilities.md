...
>
> Связано с [[tech/explanation/data-stats-damage|Explanation - Data, Stats, Damage]], [[tech/explanation/simulation|Explanation - Simulation & Tick]], [[tech/planning/phase-2-effects|Planning - Phase 2: Effects & Abilities]], [[tech/reference/combat-model|Reference - Combat Model]].
...
Вспомни граф сборок: `Data` ниже `Combat`, ссылаться вверх нельзя ([[tech/explanation/index|Explanation - Code Map]]). Но эффекту-данным нужно хранить поведение, которое оперирует боевым состоянием (`RuntimeUnit`, контекст) — а это `Combat`. Как?
...
При истечении эффект снимает свой вклад одним вызовом по ключу-источнику (`RemoveModifiersFrom(effect)`) — слоистая стат-система ([[tech/explanation/data-stats-damage|Explanation - Data, Stats, Damage]] §3.2) делает это чисто.
