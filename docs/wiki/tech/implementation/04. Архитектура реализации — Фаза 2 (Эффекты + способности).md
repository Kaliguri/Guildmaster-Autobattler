...
> Детальная архитектура **Фазы 2** из роадмапа ([9. Roadmap реализации](01.%20Roadmap%20реализации.md)): единая модель эффектов + способности.
...
| Стакинг (`StackRule`/`MaxStacks`), `EffectTagMask` | ~~Гильдмастер-кастер `GuildmasterController`~~ — **снят с дизайна (2026-07)**, см. [GDD: 0.5. Легаси](../../gdd/0.5.%20Легаси%20(снятые%20механики).md). Гильд-широкие моды → каркас-шов в Ф2, наполнение → **Фаза 5** |
