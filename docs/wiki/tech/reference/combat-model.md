...
> Опирается на принципы из [[tech/reference/tech-stack|5. Технологический стек и архитектура]]
...
> 📋 **Полный канонический список статов (ровно 30, EN+RU, операции, дефолты, клампы)** — в отдельном доке [[tech/planning/stat-system|11. Стат-система]]. Ниже — только механика стат-системы; конкретные `StatType` см. там.
...
> Полная таблица семей множителей — [[tech/planning/stat-system|11. Стат-система §5]].
...
Урон — событие, проходящее **детерминированный конвейер БЕЗ RNG** (крита и уклонения нет — решение зафиксировано). Полная формула митигации (броня = %-формула `K/(K+armor)`, пробивание = плоское + процентное) — в [[tech/planning/stat-system|11. Стат-система §4]]. Порядок (`DamagePipeline`):
```
...
Достраивает чек-лист детерминизма из [[tech/reference/tech-stack|5. Технологический стек и архитектура]].
...
`UnitSaveData` хранит `vesselId`, `equippedRelicId`, `perkIds[]`, `itemIds[]` — **строковые ID**. `RuntimeUnit` пересобирается из них + SO при загрузке, не сериализуется напрямую. См. [[tech/reference/saves|2. Сохранения]].
...
> **Решение по скоупу:** все 7 Обычных реликвий должны быть играбельны в бою к концу **Фазы 3** ([[tech/planning/roadmap|9. Roadmap §3]]), поэтому нужные им расширения затаскиваются в Фазу 3 (а не размазываются по Ф4).
...
- ~~Точный список `StatType` (~30)~~ — **решено**, см. [[tech/planning/stat-system|11. Стат-система]]. Дефолты в `StatsConfig` — баланс (Фаза 4+).
