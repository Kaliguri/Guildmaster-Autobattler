...
Урон — событие, проходящее **детерминированный конвейер БЕЗ RNG** (крита и уклонения нет — решение зафиксировано). Полная формула митигации (броня = %-формула `K/(K+armor)`, пробивание = плоское + процентное) — в [[tech/planning/stat-system|11. Стат-система §4]]:
```
