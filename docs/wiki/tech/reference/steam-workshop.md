...
> 🔴 **Контент — только ДАННЫЕ, никогда код.** Никаких DLL/скриптов в item (дыра в безопасности). Игра интерпретирует данные, а не загружает сборки. Поэтому «AI-профиль = сериализуемые данные» ([[tech/reference/tech-stack|5]], [[tech/reference/combat-model|6]]) — обязательное условие.
...
7. **Детерминизм.** Кастом-битвы/реплеи опираются на seed + детерминированную симуляцию (несущая стена из [[tech/reference/tech-stack|5]]).
...
- защищает сейвы игроков ([[tech/reference/saves|2. Сохранения]]).
