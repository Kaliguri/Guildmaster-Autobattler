...
> 🔴 **Контент — только ДАННЫЕ, никогда код.** Никаких DLL/скриптов в item (дыра в безопасности). Игра интерпретирует данные, а не загружает сборки. Поэтому «AI-профиль = сериализуемые данные» ([[tech/00-meta/journal/2026-07-30-library-picks-and-the-alternatives-we-turned-down|Journal - Library Picks And The Alternatives We Turned Down]], боевая модель (код `Assets/_Project/Scripts/Combat/`)) — обязательное условие.
...
4. **Зависимости / недостающий контент.** Профиль ссылается на юнитов/мементо по ID; у подписчика их может не быть. Нужна декларация зависимостей + мягкий фолбэк.
...
7. **Детерминизм.** Кастом-битвы/реплеи опираются на seed + детерминированную симуляцию (несущая стена из [[tech/00-meta/journal/2026-07-30-library-picks-and-the-alternatives-we-turned-down|Journal - Library Picks And The Alternatives We Turned Down]]).
...
- защищает сейвы игроков (сейвы (`CLAUDE.md` §Сохранения)).
