...
> Детальная архитектура **Фазы 2** из роадмапа ([[tech/40-planning/roadmap|9. Roadmap реализации]]): единая модель эффектов + способности.
> Наполняет швы, заложенные пустыми в Фазе 1 ([[tech/40-planning/phase-1-combat-core|10. Архитектура реализации — Фаза 1]]).
> Дизайн-источник: боевая модель (код `Assets/_Project/Scripts/Combat/`) (единая модель эффектов), [[tech/40-planning/stat-system|11. Стат-система §5]] (потенция vs длительность).
...
| Стакинг (`StackRule`/`MaxStacks`), `EffectTagMask` | ~~Гильдмастер-кастер `GuildmasterController`~~ — **снят с дизайна (2026-07)**, см. [GDD: 0.5. Легаси](../../gdd/00-meta/legacy.md). Гильд-широкие моды → каркас-шов в Ф2, наполнение → **Фаза 5** |
