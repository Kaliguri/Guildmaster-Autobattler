...
**Статус:** Дизайн-набросок (2026-07-17). Реализуется в ветке `feat/run-loop-and-simbench`. Фазы 0–2 — к постройке; «регрессия баланса в CI» сознательно вырезана (наблюдательность за балансом — на Максе). Ниже — утверждённый эскиз, по нему идёт реализация.
