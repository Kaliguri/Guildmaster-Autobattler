...
> Опирается на стат-механику из боевая модель (код `Assets/_Project/Scripts/Combat/`) (слоистые модификаторы, формула, `ScalableValue`).
> Дизайнерское описание «что это для игрока» — [GDD: 8. Статы](../../gdd/20-combat/stats.md).
...
| Поле | Значения |
|---|---|
| `DamageSchool` | `Physical` (физ) / `Magic` (маг) / `True` (чистый — игнорирует броню) |
...
| `MagicArmor` | Магическая броня | Flat | 0 | может быть `< 0` (см. §4) | ✅ |
...
> 📝 **Подумать позже:** уровни юнитов в бою (XP/Level). Сейчас прогрессия — только между боями (мементо/перки), стата Level нет. Макс не уверен — решение отложено.
...
- **Физ ветка:** `PhysArmor` / `PhysPen` / `PhysPenPct`. **Магическая ветка:** `MagicArmor` / `MagicPen` / `MagicPenPct`. **True:** митигация пропускается.
...
Ключевое разделение, не путать (исправляет раннюю формулировку боевая модель (код `Assets/_Project/Scripts/Combat/`)).
...
| Стат | Деталь |
|---|---|
| `AttackSpeed` | Атак/сек. База на мементо, бонусы через `PercentAdd` (как bonus AS в LoL). Мягкий потолок ~2.5/сек, клампится из `StatsConfig` |
