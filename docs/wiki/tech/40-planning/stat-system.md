...
> Опирается на стат-механику из [[tech/10-reference/combat-model|6. Боевая модель §3]] (слоистые модификаторы, формула, `ScalableValue`).
...
| `ElementalArmor` | Стихийная броня | Flat | 0 | может быть `< 0` (см. §4) | ✅ |
...
| `ElementalPen` | Стих. пробивание (плоское) | Flat | 0 | `≥ 0` | ✅ |
| `ElementalPenPct` | Стих. пробивание (%) | Flat | 0 | `[0, 1]` | ✅ |
...
- **Физ ветка:** `PhysArmor` / `PhysPen` / `PhysPenPct`. **Стихийная ветка:** `ElementalArmor` / `ElementalPen` / `ElementalPenPct`. **True:** митигация пропускается.
...
Ключевое разделение, не путать (исправляет раннюю формулировку [[tech/10-reference/combat-model|6. Боевая модель §5.2]]).
