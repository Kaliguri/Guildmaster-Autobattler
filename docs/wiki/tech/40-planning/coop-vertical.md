...
Хост считает бой **до конца**, потом раздаёт. Так требует дизайн пари («пока открыты ставки, лента не
уходит игрокам даже фрагментом»), и `sync-model` п.5 разрешает задержку **перед** боем; headless-прогон
1200 тиков стоит доли секунды. Внутри боя задержки нет ни у кого: каждый играет локально своими часами,
отстающий догоняет сам.
