...
> **Скоуп «закрытия Ф3» (решение Макса, 2026-06-01):** полный скоуп [[tech/40-planning/roadmap|Roadmap §3]] — **все 7 Обычных реликвий играбельны** + движковые расширения §10 [[tech/10-reference/combat-model|дока 6]], а не только срез копейщика. Шаги 5–7 ниже спроектированы design-first перед кодом (по пайплайну фазы).
...
Шаг 5 затаскивает в движок механики, которых нет в библиотеке Ф1/Ф2, но которые нужны 6 оставшимся Обычным реликвиям ([[tech/10-reference/combat-model|6. Боевая модель §10]]). Каждое расширение проектируется **сначала как шов** (сигнатура + точка интеграции + имена тестов), реализуется по мере того, как до него доходит первая реликвия в §10. Дефолты всех расширений сохраняют поведение существующего контента (копейщик/тест-юниты не ломаются).
