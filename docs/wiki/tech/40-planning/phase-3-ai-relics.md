...
| **7** | **Закрытие фазы**: все 7 срезов в харнессе, отметка в [[tech/40-planning/roadmap|9. Roadmap §3]], ретро + синхрон AI-словаря [0.3](../../gdd/relics/ai-settings-model.md) под фактический `AIProfile` | ⏳ §11 |

> **Скоуп «закрытия Ф3» (решение Макса, 2026-06-01):** полный скоуп [[tech/40-planning/roadmap|Roadmap §3]] — **все 7 Обычных реликвий играбельны** + движковые расширения §10 боевая модель (код `Assets/_Project/Scripts/Combat/`), а не только срез копейщика. Шаги 5–7 ниже спроектированы design-first перед кодом (по пайплайну фазы).
...
**Дизайн (канон — [[gdd/relics/the-spear|карточка «Железный копейщик»]]):**
...
Шаг 5 затаскивает в движок механики, которых нет в библиотеке Ф1/Ф2, но которые нужны 6 оставшимся Обычным реликвиям (боевая модель (код `Assets/_Project/Scripts/Combat/`)). Каждое расширение проектируется **сначала как шов** (сигнатура + точка интеграции + имена тестов), реализуется по мере того, как до него доходит первая реликвия в §10. Дефолты всех расширений сохраняют поведение существующего контента (копейщик/тест-юниты не ломаются).
...
> **Статус: реализовано, 144/144 EditMode зелёные** (+8 `ShepherdSliceTests`). Канон — [[gdd/relics/the-shepherd|карточка «Светлый пастырь»]]. Затянуты **§9.2** (хил-АА снарядом) и **§9.10** (глобальная одиночная активка). Ассет `LightShepherd.asset`, команда `gm_spawn_shepherd`.
>
...
- doc 13 §8: шаги 5–7 ✅; [[tech/40-planning/roadmap|9. Roadmap §3]]: отметка о завершении Ф3.
- Ретро + синхрон AI-словаря [0.3](../../gdd/relics/ai-settings-model.md) под фактический `AIProfile` (если ручки разошлись с реализацией).
