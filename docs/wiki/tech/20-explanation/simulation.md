...
**Было при заморозке:** needs_review — тик-ордер, двухфазность систем, порядок сложения в сепарации и закон видимости эффектов сверены с кодом 2026-07-28; прочее отражает код на 2026-06-19
...
ApplyDueCommands            — применить команды, чей TargetTick наступил
  ...
  ─ SpatialHash.Rebuild     — ПОВТОРНО и только если атака сдвинула тело (блинк убийцы)
  ...
  ─ ResolveCombatRounds     — применить урон/лечение реестром + доставить события; раундами,
                              пока реактивы порождают новые заявки (кап 16)
...
- **Вторая перестройка — условная и обязательная.** Блинк убийцы двигает тело уже ПОСЛЕ первой перестройки, и без повторной сетка врала бы до конца тика: соседи находили бы блинкнувшего по-разному с двух зеркальных сторон. `AutoAttackSystem.Tick` возвращает признак «сдвиг был», и только тогда хэш строится снова — в обычном тике это лишняя работа (`CombatSimulation.cs:289`).
...
Двухфазны: `MovementSystem`, `AbilitySystem`, `AutoAttackSystem`, `SeparationSystem`. Урон, лечение и
поглощение щитом копит `TickLedger` и применяет одним коммитом: щит вычитает сумму, дельты HP
складываются (успевший хилер спасает от смертельного удара), фактические числа делятся между
источниками пропорционально вкладу, убийство достаётся наибольшей доле. Переходы «за спину» —
заявка через `ICombatContext.TeleportBehind`, применяемая от общего снимка позиций.

Сторож — [[tech/40-planning/tick-resolution|Planning - Tick Resolution]] и зеркальные тесты.

#### Двухфазности мало: порядок сложения — тоже часть контракта

Двухфазность снимает зависимость от того, кто раньше в списке **применил** свой шаг. Остаётся вторая
зависимость, тоньше: в каком порядке слагаемые ложатся в **сумму**. Сложение float неассоциативно,
поэтому две одинаковые команды, отражённые по оси, получают разный результат в последнем бите — а
дальше бой раскачивает его до видимого расхождения.

Так и было в расталкивании (BAL-014: рождение на тике 68 в последнем бите X, заметность — на 116-м).
Причин было две сразу: соседи из `SpatialHash` приходят в порядке ячеек сетки, то есть растут из
координат, а у отражённых сторон порядок обратный; и взаимный зачёт половин пары
(`_push[a] += h; _push[b] -= h`) добирал юниту часть слагаемых из **чужих** итераций внешнего цикла,
причём до или после своих — смотря старшие у соседей `Id` или младшие.

`SeparationSystem` поэтому устроена так:

- **каждый юнит набирает все свои вклады сам** — пара считается дважды, по разу с каждой стороны;
  число то же (половина перекрытия), но все слагаемые приходят из своей итерации;
- **соседи упорядочены канонически и ОТНОСИТЕЛЬНО себя:** сначала свои, потом чужие, внутри группы
  по `Id`. Ключ именно относительный («свой мне / чужой мне») — только такой порядок одинаков у
  отражённых сторон, тогда как голый `Id` даёт левым `[свои, чужие]`, а правым `[чужие, свои]`.
  Упорядочивание — вставками, а не `List.Sort(Comparison)`: тот аллоцирует обёртку-компаратор на
  горячем пути, а соседей здесь единицы;
- **вырожденный случай** (тела совпали точь-в-точь, геометрия направления не даёт): направление берёт
  сторону команды у `FleeSteering.HomeDir` — единственного владельца конвенции «команда 0 слева».
  Враги расходятся каждый к своему тылу (антисимметрично само собой), свои — по `Id`, но со знаком
  стороны.

Обе первые части несущие независимо, и это замерено, а не выведено: убрать канонический порядок —
расхождение возвращается на тик 68; вернуть взаимный зачёт, оставив порядок — возвращается туда же,
бит в бит. Не «оптимизировать» обратно в один проход по `b.Id > a.Id`.

Линейка сторожей разная сознательно: `MirrorMatchTests` судит с допуском `Mathf.Approximately`, а
`MirrorBitProbe` (`[Explicit]`) — точным сравнением на полном бою, потому что допуск к дрейфу в
последнем бите слеп. Поднимать точную линейку в сторож решено НЕ надо (28.07): лишняя краснота на
безобидных для игры правках ядра дороже раннего сигнала.
