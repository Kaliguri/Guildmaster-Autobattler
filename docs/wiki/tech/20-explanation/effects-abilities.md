...
>
> Связано с [[tech/20-explanation/data-stats-damage|Explanation - Data, Stats, Damage]], [[tech/20-explanation/simulation|Explanation - Simulation & Tick]], [[tech/40-planning/phase-2-effects|Planning - Phase 2: Effects & Abilities]], [[tech/10-reference/combat-model|Reference - Combat Model]].
...
Вспомни граф сборок: `Data` ниже `Combat`, ссылаться вверх нельзя ([[tech/20-explanation/index|Explanation - Code Map]]). Но эффекту-данным нужно хранить поведение, которое оперирует боевым состоянием (`RuntimeUnit`, контекст) — а это `Combat`. Как?
...
При истечении эффект снимает свой вклад одним вызовом по ключу-источнику (`RemoveModifiersFrom(effect)`) — слоистая стат-система ([[tech/20-explanation/data-stats-damage|Explanation - Data, Stats, Damage]] §3.2) делает это чисто.

**Лакмус «стат vs эффект»:** *просто множит/прибавляет число* → стат (или эффект, кормящий стат); *реагирует на событие либо включается по длительности/условию* → эффект-компонент. Шипы реагируют → только эффект (`ThornsComponent`). Реактивный `LifestealComponent` под этой моделью **избыточен** (лечит мимо стата) — заменяется на `StatModifierComponent(+Lifesteal)`; удаление + миграция теста — [[tech/00-meta/tech-changelog|Meta - Tech Changelog & Decisions]] §3.7.
