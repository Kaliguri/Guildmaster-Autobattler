...
### Что нужно доспроектировать

1. **Что считается спором** — то есть когда кубик вообще предлагается. Предложение скрайба: кубик
   не висит кнопкой всегда, а **всплывает сам**, когда система видит расхождение (двое пометили
   разные узлы, двое заявили один предмет). Кнопка «бросим?» доступная в любой момент превращает
   каждое обсуждение в лотерею.
2. **Что если игрок отказывается бросать.** Предложение: отказ = согласие с большинством. Кубик —
   инструмент выхода из тупика, а не обязанность; молчание не должно блокировать группу.
3. **Ничья на d6.** Перебрасывают только участники ничьей, и это **бесплатно** (иначе тупик стоит
   ресурса из-за случайности, а не из-за решения).
...
Открываются за достижения — то есть по каналу «контент» (в **профиль игрока**, не за валюту дома,
2026-07-27/12). Кубик — личная вещь Гильдмастера, которую он приносит за общий стол; чужой красивый
кубик видят все. Это ровно та мелочь, которая делает ритуал ритуалом.
