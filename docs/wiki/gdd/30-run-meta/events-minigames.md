...
### Заявка: броски d6 при спорах (2026-07-29, `proposed`)

Заметка Макса: **спор между игроками решается броском d6**; за достижения открываются **скины
кубиков**. На забег выдаётся **3 кубика для переброса** (в первом акте — 1), и перебросить можно
**союзнику**.
...
*Открытое:* что именно считается спором (маршрут, награда, кому достался предмет?) и что происходит,
если игрок отказывается бросать.
