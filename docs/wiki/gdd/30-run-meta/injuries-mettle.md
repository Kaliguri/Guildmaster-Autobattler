...
> Опирается на слоистые стат-модификаторы из [[tech/planning/stat-system|Тех: 11. Стат-система]]
> и на флоу забега из [[tech/explanation/run-flow|Тех: 7. Флоу забега]].
...
- Это **макро-флоу**, не тик-симуляция → детерминизм боя не затрагивается. Роллы исходов (какая именно травма/Закалка) идут через `IRngService` (см. [[tech/reference/seed|Тех: 3. Рандомизация]]) — воспроизводимо по сиду.
...
**Откуда берётся:**
- **Поражение** в бою (попытка 1–2 из ретраев, см. [[tech/explanation/run-flow|Тех: 7 §6]]) — с «Сосудов» гильдии капает травма.
