...
/** «r,g,b» в «#RRGGBB». Нужен там, где цвет просят строкой стиля, а владеет им ST: без этого
 *  тот же оттенок приходится писать литералом второй раз, и две записи расходятся при первой
 *  же правке палитры. */
export function hexOf(rgb: string): string {
  const [r, g, b] = rgb.split(",").map((n) => Number(n.trim()));
  const two = (n: number): string => Math.max(0, Math.min(255, n | 0)).toString(16).padStart(2, "0");
  return `#${two(r ?? 0)}${two(g ?? 0)}${two(b ?? 0)}`.toUpperCase();
}
