  ...
  blocks: [
    ...
    },
    {
      ...
      items: [
        ...
        {
          id: "line", status: "accepted", tag: "форма · линия", title: "Линия",
          decision: "2026-07-31/95",
          note: "Прокол сквозь ряд: подводка идёт от юнита к концу, а не от края к центру. Форма принята, <b>кит-носитель за Максом</b> — заявка в open-forks.",
          verdict: "Второй геометрический почерк после круга полезен сам по себе: прокол читается иначе, чем взрыв вокруг.",
          draw: drawZone("line")
        },
        {
          id: "standing", status: "accepted", tag: "механика заведена", title: "Стоячая зона",
          decision: "2026-07-31/93",
          note: "Облако спор. Подводки нет — её никто не заряжает; контур держится всё время жизни, а тик виден <b>на телах</b>, не на земле.",
          verdict: "Единственный способ дать позиционную игру там, где игрок боем не управляет: решает расстановка, принятая до боя.",
    ...
    },
    {
      kind: "note",
      html:
        "Стоячая зона — <b>новая сущность симуляции</b>, а не эффект на юните: своя геометрия, своя " +
        "длительность, своё правило «кого задевает», и её надо писать в ленту боя, иначе показу нечего " +
        "рисовать. Цена названа при принятии решения.<br><br>" +
        "«Каст без cast-time» — не украшение, а найденная дыра: подводке негде жить, если урон " +
        "приходит в том же кадре. Заведена как <code>BAL-022</code>, потому что решается числом в " +
        "данных, а не рисованием."
    }
  ]
};
