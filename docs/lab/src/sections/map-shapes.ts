...
const FORMS: StandDef[] = [
  ...
  },
  {
    ...
    title: "Тропа",
    tag: "база",
    note: "Ширина два, внутри ничего не происходит. Её ценность в том, что она <b>скучная и предсказуемая</b> — иногда это ровно то, что нужно.",
    facts: [["порты", "1 → 1"], ["ширина", "2"], ["этажей", "2–4"]],
    verdict: "Принята шириной 2: единица отняла бы выбор совсем, а это привилегия застав.",
    size: [320, 250],
    draw: shape({ cols: [2, 2, 2, 2], edges: woven([2, 2, 2, 2], 31, 2) }, "1–2", "1–2")
  },
  {
    id: "lanes",
    status: "waiting",
  ...
  },
  {
    ...
    status: "waiting",
    title: "Архипелаг",
    note: "Кучки по два-три узла с пустотой между ними: густо локально, а не равномерно. Между кучками — длинные одиночные дороги.",
    facts: [["порты", "много → много"], ["этажей", "2–3"], ["чем ценен", "контраст плотности в одном экране"]],
    verdict: "Единственная форма, где разная плотность видна ВНУТРИ одного этажа.",
    size: [320, 250],
    draw: shape(
      {
        cols: [4, 4, 4],
  ...
  },
  {
    ...
    status: "waiting",
    title: "Стремнина",
    note: "Каждый шаг обязан смещать вбок в одну сторону — вернуться к центру нельзя. Куда бы ты ни вошёл, тебя сносит к одному краю.",
    facts: [["порты", "n → n, но выход предопределён"], ["этажей", "2–3"], ["образ", "река, осыпь, склон"]],
    verdict: "Самая экзотическая: её польза в предсказуемом сносе — «хочешь оказаться внизу карты, иди сюда». Кандидат на вылет.",
    size: [320, 250],
...
const PROPS: StandDef[] = [
  {
    id: "delve",
    status: "waiting",
    title: "Подземелье = остров + сокрытая",
    note: "Содержимое закрыто до входа, <b>награда объявлена заранее</b>. Внутри сразу несколько усложняющих эффектов. Собирается не правилами, а авторским подграфом из SO — процедурная карта, авторская начинка.",
    facts: [["форма", "остров"], ["свойство", "сокрытая"], ["длина", "как у соседней дороги"], ["видно заранее", "дверь и награда"]],
...
const section: SectionDef = {
  ...
  blocks: [
    ...
    },
    {
      kind: "note",
      html:
        "Обсуждение идёт, вердиктов почти нет: приняты пока поле, остров и тропа, отклонён мост. " +
    ...
    { kind: "stands", items: FORMS },
    {
      ...
      id: "props",
      title: "Свойства",
      lede: "Навешиваются на любую форму. Так «подземелье» перестаёт быть отдельной сущностью."
    },
    {
      kind: "table",
