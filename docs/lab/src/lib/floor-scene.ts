/* Сцена «пола арены»: геометрия плиты, облачное поле, небо по времени суток, вода, фактура и
   архетипы форм. Всё, из чего собираются стенды двух разделов — «Пол арены» и «Небо и вода».

   Вынесено потому, что раздел разросся до 2399 строк и двенадцати блоков: земля и небо читались
   одной простынёй, хотя решения по ним разные. Разделов стало два, а рисование у них общее —
   небо стоит за плитой в обеих сценах, и второй копии ему быть нельзя.

   ВАЖНО: сцена детерминирована по кадру и сиду. Меняя здесь хоть множитель, помни, что часть
   картинок отсюда Макс уже принял. */
...
export const U = 32;
export const ARENA_W = GEOM.arenaW * U;
export const ARENA_H = GEOM.arenaH * U;
export const HUMAN_H = GEOM.humanH * U;
export const HUMAN_W = GEOM.humanW * U;

/** Базовая толщина обводки ПРЕДМЕТОВ. У земли обводки нет вовсе. */
export const LINE = Math.max(1.5, HUMAN_H * 0.04);

/* ---------- цвет ---------- */

export type RGB = [number, number, number];

export const rgb = (c: RGB) => `rgb(${c[0] | 0},${c[1] | 0},${c[2] | 0})`;

/** Обводка предмета: затемнённый тон самой заливки со сдвигом в холод. Не чёрный. */
export const ink = (c: RGB, k = 0.4): RGB => [c[0] * k, c[1] * k, c[2] * k + 8];

export const lighten = (c: RGB, k: number): RGB =>
...
export const shade = (c: RGB, k: number): RGB => [c[0] * (1 - k), c[1] * (1 - k * 0.92), c[2] * (1 - k * 0.72) + k * 22];
...
export function vnoise2(x: number, y: number, salt: number): number {
...
export function fbm2(x: number, y: number, salt: number, octaves = 4): number {
  let sum = 0;
  let amp = 0.5;
  let norm = 0;
  let fx = x;
...
export const MORNING: TimeOfDay = {
...
export const TIMES = [MORNING, DAY, SUNSET, DUSK];
...
export const CLEAR: Weather = { id: "clear", name: "Малооблачно", clouds: 3, cloudAlpha: 0.5, shadowAmount: 0, storm: false };
export const NORMAL: Weather = { id: "normal", name: "Обычно", clouds: 7, cloudAlpha: 0.66, shadowAmount: 0, storm: false };
export const CLOUDY: Weather = { id: "cloudy", name: "Многооблачно", clouds: 14, cloudAlpha: 0.82, shadowAmount: 0, storm: false };
export const STORM: Weather = { id: "storm", name: "Гроза", clouds: 18, cloudAlpha: 0.92, shadowAmount: 0, storm: true };

export const WEATHERS = [CLEAR, NORMAL, CLOUDY, STORM];
...
export function cloudLobes(cx: number, cy: number, r: number, seed: number): Array<[number, number, number]> {
...
export function cloudSilhouette(cx: number, cy: number, r: number, seed: number): Array<[number, number]> {
...
export function cloudMass(ctx: CanvasRenderingContext2D, cx: number, cy: number, r: number, seed: number): void {
...
export function paintCloudVolume(
...
export function trailPoints(x0: number, y0: number, w: number, h: number, seed: number): Array<[number, number]> {
...
export function strokeTrail(ctx: CanvasRenderingContext2D, pts: Array<[number, number]>, width: number, seed: number): void {
  ctx.lineCap = "round";
  ctx.lineJoin = "round";
...
export function slabRockPath(ctx: CanvasRenderingContext2D, cx: number, cy: number, r: number, seed: number): void {
...
export function grassTuft(ctx: CanvasRenderingContext2D, x: number, y: number, s: number, seed: number, color: RGB, leaves = true): void {
...
export function shardShape(ctx: CanvasRenderingContext2D, x: number, y: number, s: number, seed: number, color: RGB): void {
...
export function boneShape(ctx: CanvasRenderingContext2D, x: number, y: number, s: number, seed: number, color: RGB): void {
...
export function logProp(ctx: CanvasRenderingContext2D, x: number, y: number, len: number, seed: number, color: RGB): void {
  const th = len * 0.17;
...
export function crackProp(ctx: CanvasRenderingContext2D, x: number, y: number, len: number, seed: number, dark: RGB, glow: RGB | null): void {
  const seg = 5;
...
export function pillarProp(ctx: CanvasRenderingContext2D, x: number, y: number, h: number, seed: number, color: RGB): void {
...
export function blobPath(ctx: CanvasRenderingContext2D, cx: number, cy: number, rx: number, ry: number, wobble: number, seed: number): void {
  const steps = 44;
  ctx.beginPath();
  for (let i = 0; i <= steps; i++) {
...
export function drawRim(ctx: CanvasRenderingContext2D, b: Biome, x0: number, y: number, w: number, hh: number, digital: boolean, seed = 1): void {
...
export function unitFigure(ctx: CanvasRenderingContext2D, x: number, groundY: number, lit: boolean): void {
  const hh = HUMAN_H;
  const ww = HUMAN_W;
...
export function slab(o: SlabOpts): DrawFn {
...
export const FULL: [number, number] = [760, 560];

export const NAIVE_STAND: StandDef = {
  id: "naive",
  status: "rejected",
...
export const MAIN_STAND: StandDef = {
  id: "storybook",
  status: "waiting",
...
export const RIM_STANDS: StandDef[] = [
  {
    id: "thin-digital",
    status: "accepted",
...
export const LAYER_SHOWCASE: Array<{ key: "masses" | "boulder" | "colorAccent" | "edgeShade" | "leaves" | "grain" | "props"; title: string; note: string }> = [
...
export const SHOWCASE_STANDS: StandDef[] = [
  {
    id: "base",
...
export const FIELD_STANDS: StandDef[] = [
  {
    id: "clouds-lobes",
    status: "waiting" as const,
...
export const OUTLINE_STANDS: StandDef[] = [
  {
    id: "cloud-no-outline",
    status: "waiting" as const,
    title: "Без контура",
...
export const TIME_STANDS: StandDef[] = TIMES.map((t) => ({
  id: `tod-${t.id}`,
  status: "waiting" as const,
  title: t.name,
  facts: [
...
export const WEATHER_STANDS: StandDef[] = WEATHERS.map((wv) => ({
  id: `wx-${wv.id}`,
  status: "waiting" as const,
  title: wv.name,
...
export const COMBO_STANDS: StandDef[] = [
...
export function waterProbe(withReflection: boolean): DrawFn {
  return (ctx, w, h) => {
    const b = MEADOW;
    ctx.fillStyle = rgb(b.ground);
    ctx.fillRect(0, 0, w, h);
...
export const WATER_STANDS: StandDef[] = [
  {
    id: "water-flat",
    status: "note",
...
export const SEEDS = [11, 27, 43];

export const SEED_STANDS: StandDef[] = SEEDS.map((s, i) => ({
  id: `seed-${s}`,
  status: "waiting" as const,
  title: `Сид ${s}`,
...
export const BIOME_STANDS: StandDef[] = [MEADOW, FOREST, CAVE, ASH].flatMap((b) =>
  SEEDS.map((s, i) => ({
    id: `${b.id}-${s}`,
    status: "waiting" as const,
