...
| **Ф7 Доки (D)** | ✅ влито | `79add546` | tech-scribe: новый reference ui-navigation + фикс input-camera + changelog + MOC |

**► РЕВОРК UI ЗАВЕРШЁН (Ф0–Ф7, 2026-07-19).** Все фазы влиты, K8 закрыт, доки синхронизированы.
Дальше — треки (Х/Т/Д/К/В), отдельные от реворка. Долг вне контура: битая gdd-ссылка `[[audio-subbuses]]`
в `backlog-audio-sfx.md` (заметка не создана) роняет `check-wiki-links` — чинит gdd-scribe/audio, не реворк.
...
**play-QA Макса ПРОШЁЛ (2026-07-19, «всё проверил, стабильно»):** блок B (боевой ввод-поток после K8) подтверждён.
