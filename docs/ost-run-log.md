...
| 01-A v3 стаккато-струнные + гитара | 1 | 60 | | | | |

### Что уже установлено ушами (два прогона — не зря)

- **Гитара как добавка работает.** Не лид, не рифф — второй слой поверх. Больше не переменная.
- **Флейта и соло-скрипка — вон.** Легато-носители темы дают балладу и «стандартный фэнтези вайб».
- **Боевое делает артикуляция, а не регистр.** Верх нужен стаккатный (секция короткими штрихами),
  а не поющий длинными нотами. Это отменяет мою же гипотезу «высокие тембры = эльфийское боевое».
- **Слово `fantasy` в промпте тянет в генерик.** Реф — аниме-скор, так и называть.
- **Suno сам строит длинное интро** — глушить явно (`no intro`).
