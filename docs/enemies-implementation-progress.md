...
- [ ] Фаза 2D: проказники. Белый — **дизайн-проход сделан, ждёт приёмки Макса**: карточка
      `goblin-duster` (`impl: paper`), варианты титула и стат-блок в `open-forks` §2.5. Синий ждёт
      вопроса Арканы (мана-дрейн отложен). ← ТЕКУЩЕЕ: нужен вердикт, дальше — ассет и AI-пресет.
