...
- [x] **Фаза 2C-2: слепота** (`31852cfc2` + доки `26f4b1a2a`). Примитив по ГДД: промах каждой X-й атаки,
      глубина стаками 1-4 (период 4/3/2/1), стаки живут порциями. Новый шов `IAttackMissComponent`,
      счётчик взмахов на юните, ассет `effect.blind` (3 сек, до 4 стаков). Тесты `BlindTests` — 4/4.
      Побочно исправлено: порция БЕЗ силы теперь законна — раньше порционный эффект без урона не копился.
- [ ] Фаза 2C-3: процентное пробитие брони (вердикт Макса — заводить) + разгон наездника на нём.
      ← ТЕКУЩЕЕ
- [x] **Фаза 2C-3: процентное пробитие + разгон наездника** (`74bf75340`). Пробитие — разовое свойство
      удара, проценты умножаются остатками. Разгон: проход сквозь строй, урон от скорости (`×4.95` при
      `3.63`), игнор половины брони. Тесты `ChargeThroughTests` — 4/4.
- [x] **Фаза 2C-4: сферы шамана и цикл голема** (`aa50e0400`, `3e43bea00`, `e2f3f38fe`, `3658c10ca`).
      Сферы = заряды эффекта (3 шт., откат 4 с каждая, `×2` по площади, `+100%` скорости отдельным
      эффектом). Цикл голема — один счётчик по ВЗМАХАМ с несколькими выходами. Замер: `120, 120, 240`
      с отбросом и замедлением; сферы дают три удара по 180 против обычных 90.
      Побочно исправлена общая ловушка: заряд без тега снятия диспелил ВСЁ с носителя.
      Тесты `AttackCycleTests` — 2/2.
- [x] **Фаза 2C-5: швырок союзника** (`aa50e0400`, `3e43bea00`). Урон поровну обоим, стан 1 сек. Замер:
      грант уходит в полёт, стан на нём и на цели. Тестом пока не закрыт — проверен трассой.
- [x] **Фаза 2D: белый проказник — «Гоблин-шутник»**, принят Максом (журнал ГД, `2026-07-31 (вечер, 23)`).
      Титул из трёх вариантов выбрал он; мой приоритет «целить в сильнейшего» **отклонён** — цель
      ближайшая, как у прочих гоблинов, поэтому взят готовый `ai_preset.goblin_archer`. Статы по каскаду
      без правок: HP 519, скорость 2.48, урон 30, темп 0.7, дальность 6. «Горсть в глаза» = два наложения
      `effect.blind` (общий примитив даёт стак за наложение), кд 8 с. Замер: на цели два стака, три
      промаха из девятнадцати взмахов.
- [x] **Фаза 2E: синий проказник — «Гоблин-кривляка»** (`a2152c6d7`), принят Максом (журнал ГД,
      `2026-07-31 (вечер, 26)`). Канал 4 сек режет набор ресурса цели на `-4` из базовых `5`, откат 6 сек;
      цель — ближайший ИЗ носителей маны. Потребовал двух примитивов: `StatType.ResourceRegenFlat` (32) и
      `AbilityTargetMode.NearestEnemyWithResource` (6). Замер: набор 1/сек против 5, дебафф гаснет через
      1.5 сек после обрыва канала. Тест `ManaDrainTests` (5/5). **Ростер карточных врагов закрыт: 15 из 15.**
- [x] **Фаза 3: забытые заявки разбойников** (`f29aab2dc`, 2026-08-01). Дуэлянт и убийца в движке —
      оба стояли заявками с 30.07 и оба опирались на примитивы, уже написанные и лежавшие БЕЗ носителя:
      парирование (31.07) и кровотечение (30.07). Стилер предметов отклонён Максом («Его НЕ делаем»).
      Новый код: `ParryComponent._onParryReward` и `DuelDamageBonusComponent`, тест `DuelBonusTests` (3/3).
      Замер: дуэлянт 111 в дуэли против 74 в свалке, щит 3 сек; убийца 700 колющим + 161 кровью,
      порции до четырёх. **Карточных врагов 17.**
- [ ] Тест на швырок: механика проверена трассой, своим тестом не закрыта (сферы и цикл — закрыты).

### Чего этот заход стоил из-за вранья документации

Оба разбойника были отложены по неверным причинам, и обе — из доков, а не из кода:

- «Кровотечения в движке нет» — числилось в `effects/implementation-status.md` со 29.07, хотя кровь
  завели 30.07. Поймал Макс: «Кровотечение уже есть в движке, изучи и запиши себе».
- «Фракция бандитов целиком на бумаге» и «виды bandits/beasts/golems только дизайн» — обе строки
  устарели 31.07, то есть в моём же заходе.

Отсюда правило в память (`bleed-is-in-the-engine`, `yaml-asset-grep-case-insensitive`): состояние
движка проверять по коду и ассетам, а список «чего движок не умеет» читать как подозреваемого.

### Чем синий обернулся на самом деле

Отсрочка была построена на неверной посылке. В заходе значилось «часть китов безресурсна by design,
сначала решить как дизайн» — а на диске пул ресурса задан **у всех девятнадцати реликвий** и равен
стоимости активки, то есть мана в игре работает как второй кулдаун. Мой доклад «маны нет ни у кого»
родился из регистрозависимого грепа (`stat: 27` вместо `Stat: 27`); Макс это сразу поймал. Урок общий:
ассеты YAML-ом ищутся ТОЛЬКО без учёта регистра.

Настоящих развилок оказалось четыре, и все ушли Максу: чем выражать «-4 единицы» (плоский стат против
множителя `ResourceGainEff`), какой силы дебафф, кого выбирать целью при безресурсных бойцах, и как долго
держать канал. Вердикты — в журнале ГД `2026-07-31 (вечер, 26)`, техническая сторона — запись
`2026-07-31-resource-regen-gets-a-flat-stat-and-a-target-filter`.

Осталось от него в долг: замер плотности канала — `BAL-023` (откат тикает от старта каста, поэтому
передышка выходит 2 секунды вместо 6, и это ближе к отклонённому «держит постоянно»).

### Вердикты Макса по Фазе 2C (2026-07-31, вечер)

| Развилка | Решение |
|---|---|
| Слепота | по ГДД: каждая X-я мимо; глубина стаками 1-4, порции; проказник кладёт 2 стака |
| Игнор 50% брони у разгона | завести **процентное пробитие** как общий примитив |
| Швырок союзника | урон **поровну обоим**, стан **1 сек** |
| Сферы шамана | **как в карточке**: запас 3, откат по одной раз в 4 сек, фикс +100% скорости |
