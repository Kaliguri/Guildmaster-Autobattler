...
> 📋 **Полный канонический список статов (ровно 30, EN+RU, операции, дефолты, клампы)** — в отдельном доке [11. Стат-система](11.%20Стат-система%20(полный%20список%20статов).md). Ниже — только механика стат-системы; конкретные `StatType` см. там.
...
> Полная таблица семей множителей — [11. Стат-система §5](11.%20Стат-система%20(полный%20список%20статов).md).
...
Урон — событие, проходящее **детерминированный конвейер БЕЗ RNG** (крита и уклонения нет — решение зафиксировано). Полная формула митигации (броня = %-формула `K/(K+armor)`, пробивание = плоское + процентное) — в [11. Стат-система §4](11.%20Стат-система%20(полный%20список%20статов).md):
```
...
- ~~Точный список `StatType` (~30)~~ — **решено**, см. [11. Стат-система](11.%20Стат-система%20(полный%20список%20статов).md). Дефолты в `StatsConfig` — баланс (Фаза 4+).
