...
| **6** | **6 Обычных реликвий** вертикальными срезами в порядке нарастания движка: Пастырь → Криомант → Защитник → Следопыт → Убийца → Монах (§10) | 🟡 2/6: Пастырь ✅ (§10.1), Криомант ✅ (§10.2); далее Защитник (§10.3) |
...
> **Статус: реализовано, 144/144 EditMode зелёные** (+8 `ShepherdSliceTests`). Канон — [карточка «Светлый пастырь»](Игровая%20документация%20-%20Геймдизайн/1.%20Контент%20-%20Реликвии/1.%20Обычная%20Редкость%20-%20Светлый%20пастырь.md). Затянуты **§9.2** (хил-АА снарядом) и **§9.10** (глобальная одиночная активка). Ассет `LightShepherd.asset`, команда `gm_spawn_shepherd`.
>
