**Статус:** ⚠️ Перекрыт событиями (сверено 2026-07-08). Составлен 2026-07-08, но baseline оказался неверным:
> - **Сессия A (asmdef) — уже сделана** в `master` с 2026-05-28 (`ecf9257`/`4aa01e5`). Утверждение ниже «asmdef-файлов в проекте нет» — ложь; см. вики `1. Сборки (Assembly Definitions).md`. EditMode 91/91 зелено.
> - **Сессия B (визуальный харнесс) — уже построена, но НЕ в `master`**: живёт неслитой на ветках `dev` / `feature/ai-engine` / `feature/phase3-relics` (играбельный BattleScene `8cd5c69`, спрайтовый `UnitView` `2deca27`, AI-мозг `00a2430`, вертикальный срез «Железный копейщик» `963278b`+`ed9743e`, двухфазная атака `6077279`, двойной HP-бар `77b61c4`). На `master` — пусто (0 префабов/SO/спрайтов).
> - Реальная задача теперь — **не строить с нуля, а интегрировать зрелое с веток в `master`**, не потеряв боевые фиксы самого `master` (реген/вампиризм `1b58857`, OnExpire `869a6ea`, анти-лавина `3e280e7`). Ветки и `master` разошлись — см. решение в памяти.
