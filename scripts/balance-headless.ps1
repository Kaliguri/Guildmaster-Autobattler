...
param(
    ...
    # Вместо круга — разбор ОДНОГО боя лентой событий: имена ассетов (мементо + энкаунтер, два
    # мементо или сценарий). Маркер прогона при этом не ставится: трейс не замер, а диагностика.
    [string[]]$Trace
)
