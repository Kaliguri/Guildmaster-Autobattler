...
| **Newtonsoft.Json** 3.2.2 | да | Сериализация сейвов (`JsonFileSaveService`). Нужен доступ к дереву JSON: версия схемы читается до разбора, миграции — переливка узлов. Авто-ссылается на все сборки (`isExplicitlyReferenced: 0`), правка asmdef не нужна. Готча: `Vector2` требует конвертера, иначе рекурсия по `normalized`. |
...
> **Живой и единственный сейв — `JsonFileSaveService`** (Newtonsoft, каталог `Saves/` под корнем `GameDataPath`, атомарная запись через временный файл + `.bak`, битый файл в `.corrupt`) за интерфейсом `ISaveService`. **Корень — `LocalLow/Alebardium/Guildmaster/`, а НЕ `persistentDataPath`:** последний растёт из `productName`, и переименование игры унесло бы сейвы игроков вместе с маской Steam Cloud. Кодовые имена в `GameDataPath` не менять никогда (инвариант держит `GameDataPathTests`). Сохраняем **данные, а не объекты**: durable-состояние забега — плоский DTO по строковым id, боевые сущности собираются из него фабрикой. Поэтому сильные стороны ES3 (графы объектов, ссылки на `UnityEngine.Object`, полиморфизм) адресуют проблему, которой у нас нет, — решение 2026-07-26: бэкенд свой.
>
> **Ключ сейва — это путь** внутри дерева: `prefs`, `profiles/{id}/profile`, `profiles/{id}/guilds/{gid}/run`. Каждый файл — конверт (`schemaVersion` + `gameVersion` + `payload`); версия схемы объявляется атрибутом `[SaveSchema]` на DTO, а не полем внутри состояния. Загрузка возвращает `SaveLoadResult<T>` с исходом: `Ok` / `Missing` / `Corrupted` / `TooNew` (сейв из более новой версии игры — **не грузим и не затираем**) / `Unsupported`. Полное ТЗ и фазы — [`40-planning/save-system`](docs/wiki/tech/40-planning/save-system.md).
>
> Steam Cloud за это не платит: Auto-Cloud синхронизирует файлы по маске пути и не знает, кто их писал. Маска — `Saves/**/*.json`, поэтому служебные суффиксы идут ПОСЛЕ расширения (`run.json.bak`) и в облако не едут. Ближайший долг слоя — миграции (фаза C) и мета-слой профилей и гильдий (фаза B).
>
> **Данные компьютера — отдельное хранилище:** `ILocalSaveService` → `Local/` (вне облачной маски). Там лежит `machine.json` — разрешение, режим окна, частота обновления, за интерфейсом `IDisplayService`. Синхронизировать их нельзя: чужое разрешение на втором ПК в худшем случае даёт чёрный экран. **Готча:** частоту обновления Unity умеет менять только в эксклюзивном полноэкранном; и Unity сам пишет разрешение в реестр Windows до первой сцены — владельцем остаётся наш файл, потому что мы применяем поверх.
