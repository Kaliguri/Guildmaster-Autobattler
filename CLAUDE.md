...
> **Живой и единственный сейв — `JsonFileSaveService`** (Newtonsoft, каталог `Saves/` под `Application.persistentDataPath`, атомарная запись через временный файл + `.bak`, битый файл в `.corrupt`) за интерфейсом `ISaveService`. Сохраняем **данные, а не объекты**: durable-состояние забега — плоский DTO по строковым id, боевые сущности собираются из него фабрикой. Поэтому сильные стороны ES3 (графы объектов, ссылки на `UnityEngine.Object`, полиморфизм) адресуют проблему, которой у нас нет, — решение 2026-07-26: бэкенд свой.
>
