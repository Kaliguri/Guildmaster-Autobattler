...
namespace Guildmaster.Data.Definitions
{
    ...
    public readonly struct OpenLoadoutRequest
    {
        ...
        /// <summary>Текущий надетое Мементо (для подсветки в гриде), может быть null.</summary>
        public readonly RelicData CurrentRelic;

        /// <summary>Сосуд юнита (опц.), сохраняется при смене Мементо.</summary>
    ...
    /// <summary>
    /// Запрос надеть Мементо на юнита (публикует loadout-UI по «Принять»/«Сохранить», слушает фаза
    /// расстановки — она владеет ростером и пересобирает превью). Мементо = весь боевой кит (вики «13» §3.1).
    /// </summary>
    ...
    /// <summary>Фаза перетаскивания карточки мементо из инвентаря к юниту на поле (QA #5).</summary>
    public enum RelicDragPhase { Start, Move, Drop }

    /// <summary>
    /// Перетаскивание карточки мементо из грида инвентаря на юнита в мире (QA #5). Публикует UITK-грид
    /// (pointer-capture на карточке), слушает фаза расстановки: пока тащим (Start/Move) рисует world-призрак
    /// силуэта мементо у курсора (единый вид «в руке», как drag юнита), на Drop надевает мементо на юнита
    /// под курсором. Позицию курсора фаза расстановки берёт из своего источника ввода (тот же, что deployment-
    /// pick), поэтому событие несёт лишь мементо и фазу — UI не тащит камеру/пикинг/конверсию координат.
    /// </summary>
