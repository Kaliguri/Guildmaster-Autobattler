...
{
    /// <summary>Режим авто-атаки: обычный урон, хил-автоатака (цель-союзник + Heal вместо урона) или её отсутствие.</summary>
    public enum AutoAttackMode
    {
        ...
        /// <summary>
        /// Авто-атаки нет вовсе: юнит никогда не входит в замах и не наносит ударов сам по себе.
        /// </summary>
        /// <remarks>
        /// <b>Отдельный режим, а не «урон 0»</b> (вердикт Макса 2026-07-30 по Пожирательнице снов). Ноль
        /// урона оставляет юнита в ритме атак: он подходит на дистанцию, машет, тратит замах и хвост — то
        /// есть ведёт себя как боец, просто безвредный. Для кита, чей весь урон живёт в способности, это
        /// и врёт визуально, и портит замер: DPS-метрика видит удары, которых по дизайну быть не должно.
        /// <para>Второй потребитель уже описан — Барабанщик (<c>The Cadence</c>), который «не атакует сам».</para>
        /// </remarks>
        None = 2,
    }
    ...
    /// Состав ручек продиктован ГД-моделью (блоки A–G). Мементо включает свой поднабор.
