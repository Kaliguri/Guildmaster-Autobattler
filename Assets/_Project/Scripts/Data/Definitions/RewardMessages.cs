...
{
    /// <summary>
    /// Запрос показать экран награды после боя (публикует флоу забега, слушает UI). Несёт витрину Мементо
    ...
    {
        /// <summary>Витрина: 1..N Мементо на выбор (обычно 3).</summary>
        public readonly IReadOnlyList<RelicData> Choices;

        /// <summary>Полон ли запас Мементо: если да — взять награду можно, только сбросив один существующий.</summary>
        public readonly bool InventoryFull;

        /// <summary>Текущий запас Мементо (content id) — для выбора, что сбросить при полном инвентаре.</summary>
        ...
        /// решения. Пока здесь был колбэк результата, первый нажавший забирал мементо за группу, а
    ...
    /// ни про мементо, ни про инвентарь. Обмен поэтому едет ОДНИМ вариантом («взять это, выбросив то»),
    ...
    public static class RewardOptions
    {
        ...
        /// <summary>Взять мементо — место есть.</summary>
        public static string Take(string relicId) => relicId;

        /// <summary>Взять мементо, выбросив другую: инвентарь полон.</summary>
    ...
    {
        /// <summary>Взятое Мементо, либо null = награда пропущена.</summary>
        public readonly RelicData Chosen;

        /// <summary>Content id сброшенного ради места Мементо, либо null (место было).</summary>
