...
namespace Guildmaster.UI
{
    ...
    public static class MainMenuScreenView
    {
            ...
            Action onProfile = null)
        {
            ...
            // Версия билда. Лок-ключа намеренно нет: строка не содержит переводимых слов — это имя
            // продукта и номер из ProjectSettings, одинаковые на всех языках. Нужна, чтобы по
            // скриншоту в багрепорте было видно, на чём игрок играл.
            //
            // Имя продукта стоит В СТРОКЕ, а не подразумевается (просьба Макса 05.08.2026: «настоящее,
            // полное название версии игры»). Голое «v0.1.0» отвечает на вопрос «какой номер», но не на
            // вопрос «чего» — а скриншот из багрепорта приходит без контекста, и в кооп-тесте на двух
            // машинах рядом живут ещё и старые билды. Берём из `productName`, а не константой: имя
            // игры задаётся в ProjectSettings и уже один раз менялось.
            //
            // Номер НЕ причёсывается: локальная сборка кладёт в `Application.version` полный
            // `<bundleVersion>-dev.<sha7>`, и именно хвост с коммитом отвечает на «из чего собрано».
            // Обрезать его до «0.1.0» значит выкинуть единственную часть, которую нельзя восстановить.
            if (version != null)
            {
                version.text = UnityEngine.Application.productName + " v" + UnityEngine.Application.version;
            }
