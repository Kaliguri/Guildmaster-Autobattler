...
namespace Guildmaster.Guild
{
    ...
    /// вместимости коллекции Мементо (план 11 §5.4). Три точки автосейва (вики «7» §5) зовут <see cref="Autosave"/>
    /// на переходах флоу.
    /// </summary>
    /// <remarks>
    ...
    public sealed class RunStateService : ISessionRunState
    {
        ...
        /// <summary>Начать новый забег: свежий <see cref="RunState"/> с базовой вместимостью Мементо из конфига.</summary>
        ...
        /// <see cref="GameConfig.GuildSize"/> одинаковых сосудов, у каждого базовое Мементо (пустой кит);
        /// прогрессия — мементо, которые игрок навешивает на них в лоадауте. Сосуд-контента пока нет →
        ...
        // ── Вместимость коллекции Мементо (план 11 §5.4) ─────────────────────

        /// <summary>Полон ли запас Мементо (нельзя взять награду без сброса).</summary>
        public bool RelicInventoryFull =>
            Current != null && Current.RelicInventory.Length >= Current.RelicCapacity;

        /// <summary>
        /// Добавить Мементо в запас, если есть место. false = полно (игрок должен сбросить/пропустить).
        ...
        /// <summary>
        /// Убрать один экземпляр Мементо из запаса (сброс ради места под награду).
        ...
        // ── Лоадаут: надеть/снять Мементо на сосуд гильдии (кольцо мементо, Фаза 2) ──

        /// <summary>Id «пустого» кита (базовое Мементо). Из конфига, дефолт <c>relic.base</c>.</summary>
        private string BaseRelicId => string.IsNullOrEmpty(_config.StartingRelicId) ? ContentIds.BaseRelic : _config.StartingRelicId;

        /// <summary>
        /// Надеть Мементо из запаса на сосуд слота (лоадаут-хаб): Мементо снимается с запаса и встаёт на слот, а
        /// прежний кит слота (если не базовый) возвращается в запас — это свап, не потеря. Требует, чтобы
        /// <paramref name="relicId"/> лежал в запасе. false = нет забега / слот вне ростера / Мементо нет в запасе.
        /// </summary>
        public bool EquipRelic(int slotIndex, string relicId)
        {
            ...
            if (!inv.Remove(relicId)) return false; // Мементо должен лежать в запасе
        ...
        /// <summary>
        /// Снять Мементо со слота обратно в запас; слот возвращается к базовому киту. No-op, если на слоте уже
        /// базовый кит. false также при полном запасе (некуда вернуть — Мементо не теряем).
        /// </summary>
        ...
        // Отдельно от EquipRelic/UnequipRelic: там лоадаут-хаб гоняет мементо ЧЕРЕЗ запас (свап со списанием),
        ...
        /// <summary>
        /// Поставить кит на сосуд НАПРЯМУЮ, минуя запас (drag мементо на юнита в расстановке): запас не
        ...
        // Лимита слотов нет намеренно: Party Item копится весь забег, как мементо в Slay the Spire
