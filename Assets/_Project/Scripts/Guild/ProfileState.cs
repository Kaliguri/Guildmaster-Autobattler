...
namespace Guildmaster.Guild
{
    ...
    public sealed class ProfileState
    {
        ...
        // ── Статистика профиля ──────────────────────────────────────────────
        // Заведена 21.08.2026 по заказу Макса: экран выбора профиля показывает справа СТАТИСТИКУ.
        // Копится только то, что иначе не восстановить: число гильдий и открытия считаются на лету
        // из уже существующих данных, а время и забеги знать неоткуда, если их не считать.

        /// <summary>Сколько всего наиграно этим профилем, в секундах.</summary>
        /// <remarks>
        /// Секунды, а не тики: значение переживает смену частоты симуляции и читается глазами в
        /// отладке. Копится, пока профиль активен, и сохраняется редко — раз в минуту и при выходе,
        /// чтобы не писать файл каждый кадр.
        /// </remarks>
        public long PlayedSeconds;

        /// <summary>Когда этим профилем играли в последний раз (ISO-8601 UTC). Пусто — ещё ни разу.</summary>
        public string LastPlayedUtc = string.Empty;

        /// <summary>Сколько забегов ЗАВЕРШЕНО — победой или поражением, но не брошено на середине.</summary>
        public int RunsFinished;

        /// <summary>Сколько из них выиграно.</summary>
        public int RunsWon;

        /// <summary>Лучший забег: наибольшее число пройденных узлов за один заход.</summary>
        /// <remarks>
        /// Узлы, а не акт: акт — грубая мера (их всего несколько), и «дошёл до второго» ничего не
        /// говорит о глубине. Узлы сравнимы между забегами любой длины.
        /// </remarks>
        public int BestRunNodes;
    }
