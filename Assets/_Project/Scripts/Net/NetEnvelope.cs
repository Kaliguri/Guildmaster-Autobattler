...
namespace Guildmaster.Net
{
    ...
    public enum NetChannel : byte
    {
        ...
        /// <summary>
        /// Состав сеанса: гость представляется именем, хост объявляет всем таблицу «кто, за какую
        /// сторону, каким цветом».
        /// </summary>
        SessionRoster = 11,

        /// <summary>
        /// Намерение игрока на арене: «поставь этого бойца сюда». Гость → хост.
        /// </summary>
        /// <remarks>
        /// <b>Не команда забега, и потому свой канал.</b> Команда меняет durable-состояние и обязана
        /// пережить сессию в логе; намерение же эфемерно — оно адресует бойца на арене, которого через
        /// минуту не будет. Durable-след появляется уже у владельца, когда тот исполнит намерение и
        /// переведёт его в позицию слота гильдии.
        /// </remarks>
        DeploymentIntent = 12,

        /// <summary>
        /// Шаг узла и его содержимое: что сейчас на экране у группы. Хост → гости.
        /// </summary>
        /// <remarks>
        /// <b>Не «где мы», и потому свой канал.</b> <see cref="ActivityState"/> отвечает на вопрос про
        /// МЕСТО — мероприятие, арена, карта, двор, — и переживает любой экран внутри него. Шаг узла
        /// живёт короче места и несёт содержимое (какие три реликвии выпали), поэтому сравнивать его
        /// целиком вместе с местом значило бы гонять состав награды на каждое движение фазы боя.
        /// <para><b>Содержимое едет строковыми id</b> — как чанк ленты. Гость собирает витрину из
        /// своего реестра контента, а совпадение реестров уже проверено рукопожатием.</para>
        /// </remarks>
        SessionStage = 13,
    }
