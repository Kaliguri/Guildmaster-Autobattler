...
namespace Guildmaster.Net.Tape
{
    ...
    public sealed class TapeIntake : IDisposable
    {
        ...
        /// <summary>
        /// Попросить бой С НАЧАЛА. Зовётся один раз, когда гость входит в уже идущий бой.
        /// </summary>
        /// <remarks>
        /// <b>Почему нельзя просто начать слушать с текущего места.</b> Лента адресуется номерами
        /// чанков, а недостающим здесь считается всё от нуля до наибольшего виденного: гость,
        /// подключившийся на пятисотом, объявил бы дырами пятьсот номеров и просил бы их вечно.
        /// Получив бой целиком, он видит нумерацию непрерывной — и заодно может отмотать назад
        /// (решение Макса 04.08.2026).
        /// <para>Пустая нагрузка на канале повтора и означает «пришли всё, что есть»: номер чанка в
        /// просьбе — это «повтори вот этот», а его отсутствие — «повтори бой».</para>
        /// </remarks>
        public void RequestWholeBattle()
        {
            if (!_transport.IsRunning) return;

            _transport.Send(NetPeer.HostPeerId,
                NetEnvelope.Wrap(NetChannel.TapeResend, default, ref _envelope),
                NetDelivery.Reliable);
            ResendRequestCount++;
        }
