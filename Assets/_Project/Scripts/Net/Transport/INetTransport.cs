...
namespace Guildmaster.Net.Transport
{
    ...
    public interface INetTransport
    {
        ...
        /// <summary>
        /// С кем соединение уже стоит прямо сейчас, не считая себя. У хоста — подключённые гости, у
        /// гостя — хозяин (или пусто, пока соединение не поднялось).
        /// </summary>
        /// <remarks>
        /// <b>Спросить состав можно, а не только услышать о его смене</b> (07.08.2026). Транспорт
        /// поднимается на входе в игру, а всё, что ведёт список участников — состав сеанса, гейт
        /// готовности, — рождается позже, вместе с сеансом, и пересоздаётся на каждой смене режима.
        /// Пока существовали только события, эти списки были слепы к уже подключённому напарнику:
        /// он не появлялся ни в панели участников, ни в счёте «(N/M)», и хозяин узнавал о нём только
        /// когда тот отключался (наход. Макса на живом прогоне).
        /// <para>Список — <b>ответ на момент вызова</b>; держать на него ссылку нельзя, реализация
        /// вправе переиспользовать буфер.</para>
        /// </remarks>
        System.Collections.Generic.IReadOnlyList<int> ConnectedPeers { get; }
