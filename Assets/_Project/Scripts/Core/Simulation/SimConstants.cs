namespace Guildmaster.Core.Simulation
{
    ...
    public static class SimConstants
    {
        ...
        /// <summary>
        /// Прощающий буфер досягаемости на кадре контакта, в мировых ед. (вики «14», кайт/погоня).
        /// Замах <b>не</b> прерывается уходом цели: если к удару цель отошла в пределах этого буфера
        /// (микро-дрожание, обычный шаг) — удар засчитывается; вышла за него (блинк/рывок/спид-спайк) —
        /// вхолостую (кулдаун потрачен). Тот же буфер — предсказательный гейт старта замаха: юнит не
        /// начинает свинг, который заведомо придётся вхолостую (слой 2, «сначала подойти ближе»).
        /// <para>Расширяет только «прощение» результата, НЕ базовый радиус захвата (гейт всё равно
        /// требует быть в reach). Балансная ручка «липкости» мили против кайта — крутить под плейтест.</para>
        /// </summary>
        public const float AttackReachTolerance = 0.35f;
