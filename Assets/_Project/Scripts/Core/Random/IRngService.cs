namespace Guildmaster.Core.Random
{
    ...
    public interface IRngService
    {
        ...
        /// <summary>
        /// Переинициализировать генератор из сида — свежая детерминированная последовательность
        /// БЕЗ пересоздания сервиса. Нужно для persist-мира: боевой скоуп не умирает между боями,
        /// поэтому на каждый бой RNG пересевается суб-сидом (<c>runSeed + battleIndex + attempt</c>),
        /// иначе повторный бой пошёл бы с «уехавшего» состояния → потеря воспроизводимости и
        /// рассинхрон в коопе. Тот же сид → та же последовательность, что у свежего инстанса.
        /// </summary>
