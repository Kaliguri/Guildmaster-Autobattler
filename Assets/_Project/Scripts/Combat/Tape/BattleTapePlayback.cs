...
namespace Guildmaster.Combat.Tape
{
    ...
    public sealed class BattleTapePlayback : IStageFrameSource
    {
        ...
        /// <summary>
        /// Догонять ли фронт, отстав больше требуемого. <c>true</c> (по умолчанию) — живой бой и гость:
        /// там фронт гонит ЧУЖОЙ источник, и отставание сверх требуемого значит, что показ провис.
        /// <para><b>Файловому повтору нужно <c>false</c>.</b> Там фронт двигаем мы сами, кормя ленту на
        /// опережение впереди показа, — и «отставание» от него не провис, а ровно тот запас, который мы
        /// только что и создали. Догоняя его, показ прыгал вперёд на подачу каждый кадр и промотывал
        /// бой примерно втридцатеро (замер 04.08.2026: 25 секунд боя за секунду реального времени).</para>
        /// </summary>
        public bool CatchUpToLead { get; set; } = true;
        ...
        /// задержку там, где игрок взаимодействует, нельзя. Подтягивание выключается
        /// <see cref="CatchUpToLead"/>.
        /// </summary>
        public void SetTargetLead(int ticks)
        {
            _targetLead = ticks > 0 ? ticks : 0;

            if (!CatchUpToLead) return;
