...
namespace Guildmaster.Combat.Effects.Components
{
    ...
    public sealed class EmpowerNextAttackComponent : IRearmOnRefreshComponent
    {
        ...
        [Tooltip("Выпустить усиленный удар ВНЕ ОЧЕРЕДИ (рекаст): доигрыш текущей атаки и замах нового удара " +
                 "идут вдвое быстрее, ожидание интервала снимается. Так живут «Решительный удар», удар " +
                 "Монаха воды и выход из тени у Убийцы. Выключено = заряд ждёт своей обычной атаки " +
                 "(Драугр: каждая третья). Насколько именно ускоряет — общее число игры, не поле кита.")]
        ...
        public void OnApply(in EffectContext ctx)
        {
            ...
            if (_recastImmediately) self.RecastAttack(ctx.Combat.Tuning);
        }
