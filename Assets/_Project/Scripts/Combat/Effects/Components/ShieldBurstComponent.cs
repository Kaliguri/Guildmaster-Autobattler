...
namespace Guildmaster.Combat.Effects.Components
{
    ...
    /// <para><b>Размер щита</b> спрашивается у того, кто его поднял: <see cref="RuntimeEffect.PendingShield"/>
    /// у щитов с вычисляемой величиной («от недостающего HP», «от нанесённого урона») либо
    /// потенция×стаки у обычного <see cref="ShieldComponent"/>. Своего числа взрыв не заводит — иначе
    /// у силы кита стало бы два владельца. Берётся ПОЛНЫЙ размер, а не остаток: взрыв платит за щит,
    /// который враг был вынужден пробить.</para>
    ...
    public sealed class ShieldBurstComponent : IReactiveComponent
    {
        ...
        public void OnEvent(in EffectContext ctx, in CombatEventData e)
        {
            ...
            // Щит с вычисляемой величиной сам записал её в PendingShield; у обычного ShieldComponent
            // размер = потенция×стаки. Ноль здесь значит «этот эффект щита не поднимал».
            float size = ctx.Effect.PendingShield > 0f
                ? ctx.Effect.PendingShield
                : ctx.Potency * ctx.Stacks;
            if (size <= 0f) return;
