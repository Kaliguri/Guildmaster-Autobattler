...
namespace Guildmaster.Combat
{
    ...
    public sealed class RuntimeUnit
    {
        ...
        /// <summary>
        /// Множитель хвоста-доигрыша ТЕКУЩЕЙ атаки: <c>1</c> = обычный, <c>0</c> = снят рекастом целиком.
        /// Ставится <see cref="RecastAttack"/>, тратится вместе со свингом.
        /// </summary>
        public float SwingRecoveryMult = 1f;

        /// <summary>
        /// <b>Рекаст: следующая атака выходит вне очереди.</b> Обрезает доигрыш текущей атаки, снимает
        /// ожидание интервала и — опционально — сокращает замах следующей.
        /// </summary>
        /// <param name="recoveryMult">
        /// Во что превращается хвост текущей атаки: <c>0</c> — снят целиком, <c>0.5</c> — вдвое короче,
        /// <c>1</c> — не тронут.
        /// </param>
        /// <param name="nextWindupMult">
        /// Множитель замаха следующей атаки; <c>1</c> = обычный замах. Значения меньше единицы — «ускоренный
        /// вход», как у удара в спину.
        /// </param>
        /// <remarks>
        /// Модель Макса (2026-07-31): рекаст взводится, пока текущая атака ещё идёт. <b>Её замах доигрывает
        /// целиком</b> — занесённый удар не ускоряется никогда, иначе телеграф крадётся и парирование
        /// теряет окно; ускорить можно только хвост. Дальше пропускается очередь, и у новой атаки
        /// сокращается замах.
        /// <para><b>Снятие интервала — обязательная часть, а не деталь:</b> без него обрезанный хвост лишь
        /// удлиняет боевое ожидание, и рекаст не даёт ничего.</para>
        ...
        /// следующем тике.</para>
        /// </remarks>
        public void RecastAttack(float recoveryMult = 0f, float nextWindupMult = 1f)
        {
            float mult = recoveryMult < 0f ? 0f : recoveryMult > 1f ? 1f : recoveryMult;

            if (Phase == AttackPhase.Recovery)
            {
                // Хвост уже идёт — режем остаток на месте и НЕ запоминаем множитель: иначе он дожил бы до
                // следующего свинга и обрезал бы его доигрыш тоже, то есть рекаст стал бы постоянным
                // режимом кита. Поймано тестом RecoveryCut_IsSpentByOneSwing.
                RecoveryRemaining = (int)System.Math.Round(RecoveryRemaining * mult,
                    System.MidpointRounding.AwayFromZero);
                SwingRecoveryMult = 1f;
            }
            else
            {
                // Хвост ещё не начался (взвели в замахе) — множитель сработает на входе в него и там же
                // потратится.
                SwingRecoveryMult = mult;
            }

            AttackCooldownTicks = 0;

            if (nextWindupMult > 0f && nextWindupMult < 1f) NextWindupMult = nextWindupMult;
        }
