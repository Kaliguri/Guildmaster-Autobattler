...
namespace Guildmaster.Combat
{
    ...
    public sealed class RuntimeUnit
    {
        ...
        /// <summary>
        /// Сбросить разбег целиком: набранную долю, накопленное намерение И заряд удара с разбега.
        /// </summary>
        /// <remarks>
        /// Заряд гаснет ВСЕГДА, чем бы разбег ни кончился, — прибытием у цели или обрывом (смерть,
        /// контроль, полёт, потеря цели, каст). Раньше прибытие заряд сохраняло, и «добежал, встал,
        /// ударил» выходило ударом с разбега: юнит стоял на месте, играя клип рывка, а замах ему
        /// доставался укороченный. Удар с разбега покупается ВЪЕЗДОМ в досягаемость (замах стартует
        /// ещё в беге, см. <c>CombatPositioning.CanCloseIntoReach</c>), а не фактом, что разгон когда-то
        /// был набран.
        /// </remarks>
        ...
        /// <summary>
        /// Юнит разогнался до полного разбега и ещё не ударил: если его замах начнётся ВЪЕЗДОМ, это будет
        /// удар с разбега (свой замах из <see cref="Data.Definitions.UnitData.ChargeAttackWindupMult"/>,
        /// свой клип у показа). Взводит <c>MovementSystem</c> в момент, когда разгон набран ПОЛНОСТЬЮ —
        /// не по прибытии, потому что въездной замах стартует ещё в беге. Гаснет с концом разбега любой
        /// природы (<see cref="StopSprint"/>) и тратится первым же замахом.
        /// </summary>
        public bool ChargedAttackReady;

        /// <summary>
        /// ТЕКУЩИЙ замах начат ВЪЕЗДОМ — из-за границы досягаемости, чтобы кадр контакта пришёлся на
        /// момент, когда дистанции хватит. Такой замах не рутует юнита: остаток пути он доезжает ходом
        /// (<c>MovementSystem</c>), иначе замахивался бы в пустоту в двух шагах от цели.
        /// <para>Отдельно от <see cref="ChargedSwing"/> намеренно: въезд получает КАЖДЫЙ подбегающий
        /// мили-юнит — это про то, когда удар случается. Разбег получает только тот, кто разогнался, —
        /// это про то, каким он выходит.</para>
        /// </summary>
        public bool ChargingIn;
        ...
        /// котором взводится: движение ставит его в беге, авто-атака тем же тиком входит в замах и
        ...
        /// <summary>
        /// Сколько авто-атак юнит уже провёл за бой (считается на снятии цифр, промах тоже считается).
        /// Опора детерминированных «каждая X-я атака» — слепоты и будущей Меткости.
        /// </summary>
        /// <remarks>
        /// Счётчик живёт на ЮНИТЕ, а не в эффекте, потому что слепота копится порциями (как кровотечение):
        /// у каждой порции свой срок и свой <c>RuntimeEffect.Counter</c>, и «каждая четвёртая атака» по
        /// счётчику порции означала бы разный счёт у разных порций одного дебаффа. Атаки считает тот, кто
        /// их совершает.
        /// </remarks>
        public int AttacksMade;

        /// <summary>
        /// Радиус, в котором усиленная авто-атака задевает СОСЕДЕЙ цели (огненная сфера гоблина-мага,
        /// размашистый удар голема): 0 = удар достаётся только цели. Ставится вместе с
        /// <see cref="EmpowerDamageMult"/> и тратится тем же ударом.
        /// </summary>
        /// <remarks>
        /// Площадь принадлежит ЗАРЯДУ, а не киту, — в отличие от <c>AutoAttackShape</c>, которая описывает
        /// обычный удар носителя. Кит с одиночной атакой и заряженным взмахом по площади (голем: два удара
        /// по одному, третий размашистый) иначе не выражается вовсе.
        /// </remarks>
        public float EmpowerSplashRadius;
        ...
        /// </summary>
        /// <remarks>
        /// Живёт на юните, а не в эффекте, потому что щитов может быть несколько и они сменяются, а
        /// накопленное принадлежит НОСИТЕЛЮ: кит награждается за то, что его действительно били магией, —
        /// счётчик обязан переживать смерть отдельного щита. Обнуляется вместе с рантаймом, то есть боем.
        /// </remarks>
        public float AbsorbedByWard;
        ...
        /// <summary>
        /// Тики контактов текущего свинга от его старта — по одному на Удар Атаки. Считается один раз при
        /// входе в замах (<c>AttackTiming.ContactTicks</c>) и не пересчитывается на лету: бафф скорости,
        /// прилетевший в полёте, не должен «расклеить» уже занесённый удар.
        /// </summary>
        /// <remarks>Список живёт на юните и переиспользуется — свинг стартует редко, но каждый тик мусорить нельзя.</remarks>
        public readonly System.Collections.Generic.List<int> SwingContacts = new System.Collections.Generic.List<int>(4);

        /// <summary>
        /// Сколько контактов текущего свинга уже разрешено. <c>0</c> = свинг пустой, ни одного Удара не
        /// прошло — только такому положен рефанд кулдауна при прерывании (техдолг §3.9).
        /// </summary>
        public int SwingHitIndex;

        /// <summary>
        /// Скорость проигрывания хвоста-доигрыша ТЕКУЩЕЙ атаки: <c>1</c> = обычная, <c>2</c> = вдвое быстрее.
        /// Ставится <see cref="RecastAttack"/>, тратится вместе со свингом.
        /// </summary>
        public float SwingRecoverySpeed = 1f;

        /// <summary>
        /// <b>Рекаст: следующая атака выходит вне очереди.</b> Ускоряет доигрыш текущей атаки, снимает
        /// ожидание интервала и ускоряет замах следующей.
        /// </summary>
        /// <param name="tuning">
        /// Снимок боя: обе скорости — общие ручки игры (<c>SimTuningConfig</c>, вкладка «Рекаст»), а не
        /// поля кита. Механика одна на всех, и настраивать её врозь по источникам значило бы иметь у
        /// одного факта столько владельцев, сколько активок.
        /// </param>
        /// <remarks>
        /// Модель Макса (2026-07-31). <b>Рекаст ничего не обрезает — он ускоряет:</b> обе стадии остаются на
        /// месте и проигрываются вдвое быстрее. Разница не косметическая — снятая фаза убирает окно, в
        /// которое противник успевает ответить, а ускоренная лишь сокращает его; так у рекаста остаётся
        /// цена, и парированию есть во что целиться.
        /// <para><b>Занесённый замах не ускоряется никогда.</b> Рекаст взводится, пока текущая атака ещё
        /// идёт, и её замах доигрывает целиком: телеграф, на который рассчитывали уклонение и парирование,
        /// не крадётся задним числом. Ускорить можно только то, что ещё не началось.</para>
        /// <para><b>Снятие интервала — обязательная часть, а не деталь:</b> без него ускоренный хвост просто
        /// раньше кончится, юнит встанет в боевое ожидание, и удар всё равно придёт по расписанию.</para>
        /// <para><b>Цепочки не запрещены</b> (решение Макса): рекаст может взвести рекаст, ограничений в
        /// коде нет — сдерживают заряды и кулдауны самих источников.</para>
        /// <para>Фазу метод НЕ двигает: её владелец — <c>AutoAttackSystem</c>, и переход случится в его
        /// следующем тике.</para>
        /// </remarks>
        public void RecastAttack(in Core.Simulation.SimTuning tuning)
        {
            float speed = tuning.RecastRecoverySpeed;
            if (speed < 1f) speed = 1f;   // рекаст не может замедлить: это ускорение или ничего

            if (Phase == AttackPhase.Recovery)
            {
                // Хвост уже идёт — ускоряем ОСТАТОК на месте и НЕ запоминаем скорость: иначе она дожила бы
                // до следующего свинга и ускорила бы его доигрыш тоже, то есть рекаст стал бы постоянным
                // режимом кита. Поймано тестом RecoveryCut_IsSpentByOneSwing.
                // Пол в один тик: остаток есть, значит фаза ещё идёт — ускорение не должно её схлопывать.
                int left = (int)System.Math.Round(RecoveryRemaining / speed, System.MidpointRounding.AwayFromZero);
                RecoveryRemaining = RecoveryRemaining > 0 && left < 1 ? 1 : left;
                SwingRecoverySpeed = 1f;
            }
            else
            {
                // Хвост ещё не начался (взвели в замахе) — скорость сработает на входе в него и там же
                // потратится.
                SwingRecoverySpeed = speed;
            }

            AttackCooldownTicks = 0;

            // Замах живёт в ДЛИНАХ: его считает AttackTiming, там же лежит родственный ChargeAttackWindupMult
            // кита. Конверсия «скорость → длина» одна на игру и стоит здесь, чтобы в поле не встретились
            // две единицы измерения сразу.
            float windupSpeed = tuning.RecastWindupSpeed;
            if (windupSpeed > 1f) NextWindupMult = 1f / windupSpeed;
        }
        ...
        /// <summary>
        /// Канал авто-атаки — снимок кита на бой, как и <see cref="AutoAttackDamageType"/>. Заполняет
        /// <c>RuntimeUnitFactory</c> из <see cref="UnitData.Channel"/>.
        /// </summary>
        /// <remarks>
        /// Поле, а не чтение кита напрямую, потому что канал бывает свойством СТОЙКИ, а не кита: у
        /// Десятины поток живёт в дальней форме, а в упор она бьёт обычными выпадами. Читай кит — и
        /// переключение форм пришлось бы выражать вторым китом.
        /// </remarks>
        public Data.Definitions.AttackChannel AttackChannel;

        // Имена ниже начинаются с Attack не для красоты: канал СПОСОБНОСТИ (ChannelRemaining и родня, ниже
        // в этом же файле) — другая механика с другим владельцем (AbilitySystem против AutoAttackSystem).
        // Совпади имена — и один счётчик получил бы двух владельцев, что здесь уже объявлено дефектом.

        /// <summary>
        /// Тиков КАНАЛА авто-атаки осталось (<see cref="AttackPhase.Channel"/>). 0 = канал не идёт; у
        /// кита без канала поле не используется вовсе.
        /// </summary>
        public int AttackChannelRemaining;

        /// <summary>
        /// Тиков до следующего тика урона внутри канала. Отдельный счётчик, а НЕ
        /// <see cref="AttackCooldownTicks"/>: кулдаун отмеряет период между атаками, а здесь — период
        /// внутри одной атаки, и один счётчик на два смысла разъехался бы на первом же бафе скорости.
        /// </summary>
        public int AttackChannelTickRemaining;

        /// <summary>
        /// Цель канала: поток льётся в неё, пока она жива и в радиусе. Своё поле, а не
        /// <see cref="WindupTarget"/> — тот по контракту гаснет в резолве замаха, и канал, доживающий до
        /// следующего свинга, читал бы уже обнулённый снимок.
        /// </summary>
        public RuntimeUnit AttackChannelTarget;
        ...
        /// <summary>
        /// Фокус авто-атаки, назначенный ФОРМОЙ: <c>null</c> — фокус берётся из <c>AIProfile</c> кита,
        /// как у всех. Ставит <c>AttackStanceComponent</c>, читает мозг.
        /// </summary>
        /// <remarks>
        /// <b>Почему переопределение, а не полный снимок рядом с типом урона и доставкой.</b> Те части
        /// формы читает боевой пайплайн, поэтому боец обязан нести их сам. Фокус же читает только мозг,
        /// у которого профиль кита уже есть; сделай его полным снимком — и каждый, кто собирает бойца
        /// руками (стенд, тесты), обязан был бы продублировать фокус из профиля, а забыв, молча получил
        /// бы «ближайшего» вместо заявленного. Пустое значение означает «стойки на бойце нет» — у кита без
        /// стоек оно пустое всегда, и снятие стойки возвращает его в пустое.
        /// </remarks>
        public Data.Definitions.TargetingMode? StanceTargeting;
        ...
        /// <summary>
        /// Сильнейшая ступень Маскировки из активных эффектов носителя; <see cref="Data.Definitions.ConcealmentTier.None"/>
        /// — обычный юнит. Пишет только <c>ConcealmentSystem</c>, раз в тик и по всем источникам сразу.
        /// </summary>
        /// <remarks>
        /// Владельцем сделана система, а не сам компонент эффекта: маскировок на юните может оказаться
        /// две (пассив плюс активка), и тогда снятие одной обнулило бы поле, оставив вторую висеть без
        /// действия. Пересчёт по списку эффектов такой гонки не знает.
        /// </remarks>
        public Data.Definitions.ConcealmentTier ConcealTier;

        /// <summary>
        /// Замаскированного заметили: враг подошёл ближе радиуса его ступени. Обнаружение <b>командное</b>
        /// (решение Макса 2026-07-31) — увидел один, видит вся команда; отошли все, и юнит снова пропал.
        /// </summary>
        public bool Revealed;

        /// <summary>
        /// Юнит скрыт прямо сейчас: замаскирован и не обнаружен. Вражеский таргетинг такого не выбирает,
        /// а прилетевший в него удар гасится — уход в тень посреди чужого замаха считается уклонением
        /// (решение Макса 2026-07-31).
        /// </summary>
        public bool IsHidden => ConcealTier != Data.Definitions.ConcealmentTier.None && !Revealed;
