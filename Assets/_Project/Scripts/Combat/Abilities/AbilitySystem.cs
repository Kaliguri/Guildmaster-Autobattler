...
namespace Guildmaster.Combat
{
    ...
    public sealed class AbilitySystem
    {
            ...
            IReadOnlyList<RuntimeUnit> units, ICombatContext ctx)
        {
            ...
            // Рекаст авто-атаки (M18): умение-УДАР пускает обычную атаку вне очереди — в окне получаются
            // два удара почти подряд. Общий примитив, а не своя правка таймера: обрезать хвост, пропустить
            // очередь и (при желании кита) сократить замах — это один жест, у которого один владелец.
            // Замах здесь не сокращаем: умение уже дало свой удар, ускорять сверху нечего.
            // Канал сюда не попадает намеренно: сброс на каждом его срабатывании держал бы авто-атаку
            // взведённой всё время канала — это уже другая механика.
