...
namespace Guildmaster.Combat
{
    ...
    public sealed class ProjectileSystem
    {
        ...
        private static void ApplyHit(Projectile p, RuntimeUnit target, ICombatContext ctx)
        {
            ...
            // Пробивание — свойство СКВОЗНОГО полёта: снаряд проходит цель насквозь и летит дальше, к
            // следующей. Трекинг летит в одну конкретную цель и на попадании встаёт ровно в неё, так что
            // выживший трекинг-снаряд бил бы ту же цель каждый следующий тик: pierce+1 полных ударов и
            // столько же наложений on-hit эффектов за один выстрел. Хил-снаряды (всегда трекинг) гаснут
            // выше по той же причине.
            if (p.TargetUnit != null) { p.IsAlive = false; return; }
