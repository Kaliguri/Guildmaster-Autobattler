...
namespace Guildmaster.Presentation.Effects
{
    ...
    public static class HitFormFactory
    {
            ...
            uint seed, bool endsAtHit, float freezeSeconds)
        {
            ...
            //
            // Двойка — не запас, а геометрия: форма навылет центрирована в точке хита и уходит в обе
            // стороны, поэтому до точки A дотягивается ПОЛОВИНА длины. Форма, кончающаяся в цели
            // (принял щит), покрывает путь один в один.
            //
            ...
            if (kind == HitFormKind.Slash)
                length = Mathf.Max(length, Vector3.Distance(from, to) * (endsAtHit ? 1f : 2f));
