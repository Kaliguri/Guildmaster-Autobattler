...
namespace Guildmaster.Presentation.Body
{
    ...
    public interface IUnitBodyVisual
    {
        ...
        /// <summary>
        /// Узел, ВНУТРИ которого действует собственный порядок частей (носитель <c>SortingGroup</c>).
        /// Эффекту, которому положено место МЕЖДУ частями тела, другого способа туда попасть нет: снаружи
        /// группа сортируется целиком, и объект может лежать только над ней или под ней — но не между
        /// мечом и предплечьем.
        /// </summary>
        /// <remarks>
        /// По конвенции рига этот узел не масштабируется и не зеркалится: и разворот
        /// (<see cref="SetFlipX"/>), и сплющивание живут на <see cref="Root"/>, который лежит ниже.
        /// <c>null</c> — группы у тела нет, и вклиниваться некуда.
        /// </remarks>
        Transform SortingRoot { get; }
