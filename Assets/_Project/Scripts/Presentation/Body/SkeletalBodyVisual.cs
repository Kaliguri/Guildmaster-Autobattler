...
namespace Guildmaster.Presentation.Body
{
    ...
    public sealed class SkeletalBodyVisual : MonoBehaviour, IUnitBodyVisual
    {
        ...
        private void Awake()
        {
            ...
            // Список пуст — разводка не сделана. Собираем из иерархии, чтобы юнит не вышел на арену телом,
            // которое нечем покрасить и нечем разбить, но молчать об этом нельзя: без списка мы не знаем
            // задуманного порядка частей и берём тот, в котором их вернуло дерево.
            if (_parts.Count == 0)
            {
                CollectParts(_parts);
                Debug.LogError($"[SkeletalBodyVisual] {name}: список частей пуст — собран из иерархии " +
                               $"({_parts.Count} шт.) в порядке дерева. Разведи порядок кнопкой «Собрать заново» " +
                               "в инспекторе, иначе части рисуются в случайном порядке.", this);
            }
