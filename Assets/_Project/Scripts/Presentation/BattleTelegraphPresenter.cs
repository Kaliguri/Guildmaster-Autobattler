    ...
    public sealed class BattleTelegraphPresenter : ITickable, IDisposable
    {
        ...
        public void Tick()
        {
            ...
            for (int i = 0; i < _upcoming.Count; i++)
            {
                ...
                // Барьер анонсируется ПОЗОЙ: щит уходит вверх до того, как барьер появится, и держится,
                ...
                // хоть к яду.
                //
                // У барьера поза — ВСЯ подводка, вспышки ему не полагается (решение Макса 31.07.2026).
                // Синее моргание переехало на момент БЛОКА, где оно и означает событие: щит сработал.
                // Пока оно играло ещё и здесь, до удара, оно моргало на каждый барьер и читалось как
                // сам удар — то есть предупреждение спорило с событием, о котором предупреждало.
                ...
                // вспышку просят только эффекты без своей позы; когда появится второй такой цвет, здесь
                // встанет выбор по тегу эффекта, а не новый цвет в коде.
