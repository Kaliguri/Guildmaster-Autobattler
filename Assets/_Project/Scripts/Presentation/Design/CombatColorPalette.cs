...
namespace Guildmaster.Presentation.Design
{
    ...
    public sealed class CombatColorPalette : ScriptableObject
    {
        ...
        [Tooltip("Во сколько раз ярче базы вспышка тела в момент БЛОКА. Сила вспышки клампится к единице " +
                 "и на обычном ударе уже в потолке, поэтому «сверкнуть ярче» можно только цветом за " +
                 "порогом bloom. Оттенок тот же, что у полосы щита: блок и защита — одно событие.")]
        [SerializeField] private float _blockFlashBrightness = 2.4f;
        ...
        /// <summary>
        /// Вспышка тела в момент БЛОКА: тот же цвет щита, поднятый в HDR. Своей роли в палитре у неё нет и
        /// быть не должно — цвет защиты один, а «ярче» это сила свечения, которая в токены не едет
        /// (в USS цвета LDR, HDR накручивается здесь множителем, как у свечения юнита).
        /// <para>Ярче именно ЦВЕТОМ, а не силой вспышки: та клампится к единице и на обычном попадании
        /// уже упёрта в потолок, так что блок иначе не отличить от рядового удара.</para>
        /// </summary>
        public Color ShieldFlare => Brighten(Shield, _blockFlashBrightness);
