...
namespace Guildmaster.Presentation.Map
{
    ...
    public sealed class MapNodeView : MonoBehaviour
    {
        ...
        /// <remarks>
        /// Сейчас каждый вариант использует один нормализованный векторный силуэт. Ссылка всё равно хранит
        /// объект-корень, а не <see cref="SpriteRenderer"/>: это оставляет художнику возможность собрать
        /// составной знак позднее, не меняя формат префаба или код показа.
        /// </remarks>
        [Serializable]
        public struct IconVariant
        {
            ...
            [Tooltip("Корень иконки этого типа: сам спрайт или объект-группа. Держится выключенным, " +
                     "включается по типу узла.")]
