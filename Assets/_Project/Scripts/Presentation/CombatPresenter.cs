    ...
    public sealed class CombatPresenter : MonoBehaviour
    {
        ...
        private void HandleDamageDealt(int sourceId, int targetId, DamageResult result)
        {
            ...
            if (_views.TryGetValue(targetId, out var view) && view != null)
            {
                ...
                // Вспышка защиты играет на ФАКТ срабатывания щита, а не на его полноту: щит Оплота — 50 ед.
                // плюс доля недостающего HP, и удар целиком он съедает редко, так что по «полному блоку»
                // событие почти никогда не показывалось. Пробили щит или нет — читается остальным
                // (искры, отброс, цифра HP), а сам факт «щит сработал» обязан быть виден каждый раз.
