    ...
    public sealed class CombatPresenter : MonoBehaviour
    {
        ...
        private void HandleDamageDealt(int sourceId, int targetId, DamageResult result)
        {
            ...
            if (_views.TryGetValue(targetId, out var view) && view != null)
            {
                ...
                // Щит сработал — тело уходит в тень, щит остаётся в цвете защиты. Играет на ФАКТ поглощения,
                // а не на полный блок: щит Оплота это 50 ед. плюс доля недостающего HP против авто-атаки на
                // сотню, целиком удар он съедает редко, и по «полному блоку» событие почти не показывалось.
                if (shieldAbsorbed && _colorPalette != null)
                    view.OnShieldAbsorbed(_colorPalette.BlockFlash);

                // Удар, съеденный щитом целиком, вспышки тела не получает: вспышка означает «дошло», а сюда
                // не дошло. Пробитие показывается как обычно — своим цветом, с отбросом от бьющего.
                if (!blocked) view.OnDamageReceived(flash, nudgeDir);
            }
