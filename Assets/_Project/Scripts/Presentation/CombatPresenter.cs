    ...
    public sealed class CombatPresenter : MonoBehaviour
    {
        ...
        private void HandleDamageDealt(int sourceId, int targetId, DamageResult result)
        {
            ...
            if (_views.TryGetValue(targetId, out var view) && view != null)
            {
                ...
                // Блок берёт ФЛАР щита, а не его UI-цвет: флар HDR-яркий и за порогом bloom даёт то самое
                // сверкание. Силой вспышки этого не добиться — она клампится, и обычный удар уже в потолке.
                if (blocked && _colorPalette != null) flash = _colorPalette.ShieldFlare;
