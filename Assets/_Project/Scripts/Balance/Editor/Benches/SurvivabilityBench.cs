...
namespace Guildmaster.Balance.Editor
{
    ...
    public static class SurvivabilityBench
    {
        ...
        public static (string csv, string md) Run()
        {
            ...
            var headers = new List<string> { "Relic", "TTD_solo", "EHP_solo", "TTD_focus3", "EHP_focus3" };
            var table = new List<IReadOnlyList<object>>();

            foreach (RelicData relic in relics)
            {
                (double ttd1, double ehp1) = RunSurvival(config, relic, attackers: 1, cap);
                (double ttd3, double ehp3) = RunSurvival(config, relic, attackers: 3, cap);
        ...
        private static (double ttd, double ehp) RunSurvival(StatsConfig config, RelicData relic, int attackers, int cap)
        {
