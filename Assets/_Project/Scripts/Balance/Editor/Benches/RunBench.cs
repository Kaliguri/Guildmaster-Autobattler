...
namespace Guildmaster.Balance.Editor
{
    ...
    ///
    /// <para><b>Отряд собирается из настоящих мементо, а не манекенов</b> — см.
    ...
    public static class RunBench
    {
        ...
        /// мементо каждой роли. Сколько наберётся на деле — решает ростер тира: повторы отсеиваются,
        ...
        /// </summary>
        /// <remarks>
        /// Пока границы не было, варианты состава шли по общему списку мементо подряд: первые два
        ...
        /// <summary>
        /// Разные четвёрки одного тира: вариант <c>k</c> берёт k-е мементо каждой роли, повторы
        ...
        /// <summary>
        /// Собрать отряд НАСТОЯЩИХ мементо под строй — по одной на слот, по совпадению роли.
        /// </summary>
        /// <remarks>
        ...
        /// <para>Мементо берутся детерминированно (порядок ассетов), <c>relic.base</c> исключён: он
        /// заведомо слабейший в ростере и мерил бы не отряд, а свою болванку. Роль без мементо
            ...
            Dictionary<string, RelicData[]> heroesByMode)
        {
            ...
            const string notes =
                ...
                "Составы собираются по кругу ВНУТРИ ТИРА: вариант k берёт k-е по счёту мементо каждой " +
            ...
            List<string> missing, List<string> thinRoles, Dictionary<string, RelicData[]> heroesByMode)
        {
            ...
            sb.AppendLine();
            sb.AppendLine("**Кто дерётся.** Каждый слот строя закрыт НАСТОЯЩИМ мементо своей роли " +
            ...
            sb.AppendLine("**Отряд набирается в границах ОДНОГО тира.** Обычные и трешевые киты — разные " +
                          ...
                          "мементо. Тыловой слот принимает Поддержку, Призывателя и **Целителя** — " +
            ...
            {
                sb.AppendLine($"**Ролей без мементо: {string.Join(", ", thinRoles)}** — их слоты закрыты " +
