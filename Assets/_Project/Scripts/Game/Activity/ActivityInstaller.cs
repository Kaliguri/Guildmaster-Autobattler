...
namespace Guildmaster.Game.Activity
{
    ...
    public sealed class ActivityInstaller : IInstaller
    {
        ...
        private void RegisterBattleSeam(IContainerBuilder builder)
        {
            ...
            // Обычная регистрация, а не точка входа: точки входа диспатчатся на следующем кадре, а шов
            // «мероприятие заказывает бой» обязан стоять уже в том кадре, в котором мероприятие открыли —
            // узел заказывает первый бой сразу. Рождение владельца поручено ActivityHost.Open, снос —
            // контейнеру (он диспозит свои singleton'ы вместе со скоупом).
            builder.Register<BattleHost>(Lifetime.Singleton).AsSelf();
        }
