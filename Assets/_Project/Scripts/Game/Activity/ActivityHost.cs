...
namespace Guildmaster.Game.Activity
{
    ...
    public sealed class ActivityHost : IDisposable, IActivityView
    {
        ...
        public void Open(ActivitySetup setup)
        {
            ...
            // Владелец боя рождается ВМЕСТЕ с мероприятием, а не когда до него дойдут руки: шов
            // «мероприятие заказывает бой» он привязывает в конструкторе, и до первого резолва этого
            // шва не существует. Узел заказывает бой в том же кадре, в котором открылось мероприятие,
            // поэтому опоздание на кадр читалось как «некому запустить бой» → забег обрывался в главное
            // меню (наход. Макса 22.08.2026).
            _ = Battle;
