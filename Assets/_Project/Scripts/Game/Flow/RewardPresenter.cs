...
namespace Guildmaster.Game.Flow
{
    ...
    /// решения. Пока выбор был локальным, первый нажавший забирал мементо за группу.
    /// <para><b>Витрину катит хозяин и раздаёт результат раската</b> — гость собирает из id ту же
    /// витрину. Второй раскат у него дал бы другие три мементо: бросок случаен.</para>
    ...
    public sealed class RewardPresenter : IRewardPresenter
    {
        ...
        // Сброс мементо ради места — односторонняя запись, идёт через шину и попадает в лог. Взятие
        ...
        public async UniTask PresentAsync(RewardTier tier, CancellationToken ct = default)
        {
            ...
            if (choices.Count == 0)
            {
                Debug.LogWarning("[RewardPresenter] - пул наград пуст (нет Мементо в контент-БД) → без награды");
                return;
            }
