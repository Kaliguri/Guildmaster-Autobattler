...
namespace Guildmaster.Game
{
    ...
    public class RootLifetimeScope : LifetimeScope
    {
        ...
        [Tooltip("Снимок палитры проекта (UI/Theme/tokens.*.uss → Alebardium/Дизайн-система/Пересобрать палитру). " +
                 "Нужен интерфейсу, чтобы карточка мементо красила тело ТЕМ ЖЕ путём, что бой: юнит хранит " +
        ...
        protected override void Configure(IContainerBuilder builder)
        {
            ...
            // мементо. Без неё карточка рисует арт как нарисован, поэтому Optional, а не Require.
            builder.RegisterInstance(ScopeWiring.Optional(_palette, nameof(RootLifetimeScope), nameof(_palette),
                "карточка мементо покажет тело без приглушения — в бою оно будет, на карточке нет"));
            ...
            // Секундомер наигранного. В корне, потому что считает время С ЗАПУЩЕННОЙ ИГРОЙ, а не время
            // забега: сеанс и мероприятие приходят и уходят, часы игрока идут всё это время.
            builder.RegisterEntryPoint<Services.PlayTimeTracker>(Lifetime.Singleton);
