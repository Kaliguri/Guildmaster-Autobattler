...
namespace Guildmaster.Game.Session.Net
{
    ...
    /// <para><b>Мементо поехала тем же каналом 08.08.2026.</b> До этого возилось только перемещение,
    ...
    public sealed class DeploymentIntentSender : IStartable, IDisposable
    {
        ...
        /// <summary>
        /// Мементо едет строковым id, а не объектом: по проводу ездит контент, а не ссылки на ассеты.
        /// Совпадение реестров уже проверено рукопожатием, поэтому владелец соберёт ту же мементо.
        /// </summary>
    ...
    public sealed class DeploymentIntentIntake : IStartable, IDisposable
    {
        ...
        private void HandleMessage(int from, ArraySegment<byte> message)
        {
            ...
            // Мементо собираем из СВОЕГО реестра по id: по проводу ездит контент, а не ссылки на
            ...
            Guildmaster.Core.Diagnostics.Diag.Log(Guildmaster.Core.Diagnostics.DiagChannel.Commands,
                $"хозяин: мементо «{intent.RelicId}» нет в реестре — надеть его пиру {from} нечем");
        }
    }

    /// <summary>
    /// Разобранное намерение расстановки: либо «поставь бойца сюда», либо «надень на него мементо».
    /// </summary>
