...
namespace Guildmaster.Game.Session.Net
{
    ...
    /// отключения, плюс мы сами. Отсюда следствие: <b>уход игрока не подвешивает кнопку</b> — он
    /// перестаёт быть участником, и оставшиеся оказываются «всеми». Кнопка, которую невозможно нажать
    /// из-за того, что кто-то вышел, — худший вид зависания: причина не видна на экране.
    /// <para><b>Но набранные голоса при этом гаснут — все.</b> Опускать планку, не гася их, значит
    /// принимать решение за того, кто ждал напарника: он нажал, напарник отвалился, и выбор случился
    /// сам собой. Нажать второй раз дёшево, отменить сделанный выбор — нет.</para>
    ...
    public sealed class HostSharedDecision : ISharedDecision, IStartable, IDisposable
    {
        ...
        private void OnPeerDisconnected(int peerId)
        {
            ...
            // Гасим голоса ВСЕХ, а не только ушедшего. Иначе выходит так: оставшийся нажал и ждёт
            // напарника, напарник отваливается — планка опускается ровно до набранного, и выбор
            // делается САМ, пока игрок смотрит на «ждём второго». Пусть лучше он нажмёт ещё раз,
            // видя, что остался один (вердикт Макса 08.08.2026).
            ClearVotes();
            Announce();
        }
