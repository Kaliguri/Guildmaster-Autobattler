...
namespace Guildmaster.Game
{
    ...
    public sealed class DeploymentInteraction : IStartable, ITickable, IDisposable
    {
        ...
        /// <remarks>
        /// Ристалище и PvP отличаются ровно этим, и разница описана флагом мероприятия
        /// (<c>ActivitySetup.OwnUnitsOnly</c>), а не двумя ветками кода. Сторона своего игрока берётся
        /// у состава сеанса и НЕ считается нулём: в PvP оба клиента считали бы своей одну и ту же
        /// сторону и распоряжались бы одним строем.
        /// </remarks>
