...
{
    /// <summary>
    /// Бэкенд <see cref="ISaveService"/> — наш собственный и единственный: JSON-файл на диск
    /// (<see cref="Application.persistentDataPath"/>) через <see cref="JsonUtility"/>, который сериализует
    /// <c>[Serializable]</c>-DTO с публичными полями.
    ...
    /// которой у нас нет. Steam Cloud тоже за это не платит: Auto-Cloud синхронизирует файлы по маске
    /// пути и не знает, кто их записал.</para>
