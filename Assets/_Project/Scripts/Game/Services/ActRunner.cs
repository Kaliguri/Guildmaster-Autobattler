        ...
        public async UniTask<EventResult> RunActAsync(RunContext ctx)
        {
            ...
            try
            {
                while (!MapTraversal.IsActComplete(map))
                {
                    ...
                    // Узел пройден — раны стали на узел старше, и мелкие, чей срок вышел, сходят сами
                    // (ГДД injuries-mettle §Как снимается). Зовётся здесь, а не по таймеру или уровню:
                    // «сколько узлов прожила рана» обязано считаться там же, где узлы и засчитываются.
                    _runStates.AdvanceInjuries();
                    _runStates.Autosave();
                }
