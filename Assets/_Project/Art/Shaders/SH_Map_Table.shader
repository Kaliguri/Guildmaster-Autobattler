Shader "Guildmaster/Map/Table"
{
    ...
    Properties
    {
        ...
        // Виньетка живёт ЗДЕСЬ, а не в пост-обработке камеры: UI рисуется поверх пост-эффектов и
        // до него они не долетают, а нужен нам как раз тёмный край ПОД интерфейсом. Дефолт не 0 —
        // без затемнения углов плоский тайл читался «обоями», из-за которых меню казалось пустым.
        _Vignette ("Виньетка (тёмный край)", Range(0, 1)) = 0.6
