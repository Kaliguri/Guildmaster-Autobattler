Shader "Guildmaster/Vfx/HitForm"
{
    ...
    Properties
    {
        ...
        // Ширина перехода между ступенями (ядро → кайма → обводка), в долях толщины. Ноль — ступени
        // встык, и знак читается тремя вложенными наклейками вместо одного градиента. Внешнюю границу
        // обводки не трогает: она и есть лайн, ей резкость положена.
        _Softness ("Softness (мягкость переходов между ступенями)", Range(0, 1)) = 0.35
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 Frag(Varyings i) : SV_Target
            {
                ...
                // Мягкость переходов МЕЖДУ СТУПЕНЯМИ — в долях толщины, а не в пикселях: пиксельный
                // край даёт три встык стоящие наклейки, а знак обязан читаться одним градиентом.
                // Пиксельный минимум остаётся полом, иначе тонкая форма зазубрится.
                float soft = max(_Softness * t * 0.5, edge);

                // Переход «кайма → обводка» мягкий, а вот сама внешняя граница обводки ниже — резкая:
                // лайн обязан быть краем, а не растушёвкой (Макс, 06.08.2026).
                float mask = 1.0 - smoothstep(t - soft, t + soft * 0.4, d);
                ...
                float coreMask = (1.0 - smoothstep(coreT - soft, coreT + soft, d)) * mask;
