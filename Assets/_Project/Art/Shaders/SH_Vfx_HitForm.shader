    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 Frag(Varyings i) : SV_Target
            {
                ...
                // Прорастание от A к концу: за первую долю жизни фронт проходит форму насквозь.
                // Именно ПРОРАСТАНИЕ, а не появление целиком: удар пришёл откуда-то, и это видно.
                float grow  = saturate(_Progress / max(_Grow, 1e-4));
                float front = lerp(-1.0, 1.0, grow);
                float visible = 1.0 - smoothstep(front, front + 0.28, p.x);

                // Угасание: со второй половины жизни форма уходит целиком, сохраняя рисунок.
                float fade = 1.0 - smoothstep(_Grow, 1.0, _Progress);
