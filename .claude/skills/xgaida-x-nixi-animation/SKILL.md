...
| Композиция слоёв: база + надстройка через маску, как в Animator | `Rig/RigLayerBlend.cs` |
...
- **Композицию судить композицией.** Игра не играет один клип: щит встаёт поверх бега, свинга, разбега.
  Контактный лист умеет `Overlay` + `OverlayMask` (`RigLayerBlend`), кадрирование считается по составной
  позе. Сэмплировать два клипа подряд НЕЛЬЗЯ — второй перезаписывает риг вместе с маской.
- **Маска щита — только рука со щитом**, поэтому база (ноги, торс) продолжает своё. А вот у маски рук
  (`Mask_Arms`) торса, головы и таза нет — если переносить туда обычные атаки, вес удара пропадёт: он
  держится на наклоне корпуса и просадке таза.
