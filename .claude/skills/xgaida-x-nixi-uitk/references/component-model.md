...
### Вершинный цвет отдаётся КАК ЕСТЬ — руками в линейное не переводить

Шейдер UI Toolkit конвертирует вершинный tint сам, как Canvas у uGUI. Ручной `c.linear` поверх —
вторая конверсия подряд, и она душит цвет **втрое**: заданное `rgb(44,140,146)` приходит на экран
как `rgb(6,62,69)`. Прожило это у нас сутки и обернулось жалобой «кнопки слишком тёмные», под
которую чуть не начали перекрашивать палитру — лечили бы симптом, а причина была в одной строке.

**Цвет ВСЕГДА проверяется замером пикселя, а не взглядом на скрин.** На тёмной гамме расхождение
между «задано» и «нарисовано» выглядит как дизайнерское решение, а не как дефект. Процедура:

```python
# снять кадр, взять точку внутри элемента и сравнить с тем, что задано
im.getpixel((x, y))   # ожидали 44,140,146 — получили 6,62,69 ⇒ конверсия лишняя
```

Проба, отделяющая «цвет не тот» от «цвет не доехал»: подать предкомпенсированный `c.gamma`. Стало
ровно заданным ⇒ где-то в цепочке лишний `.linear`.
...
- ❌ Свой цвет в поле контрола вместо значения из USS (дефолт обязан быть `Color.clear`) — ловит
  `UiColorPipelineTests`.
- ❌ Ручной перевод цвета (`.linear` / `.gamma`) где угодно в UI-коде — шейдер делает это сам.
