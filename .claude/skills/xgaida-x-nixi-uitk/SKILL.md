...
| **Контактный лист состояний** | `Alebardium → UI → Contact Sheet` (в play) → `docs/lab/assets/ui-states` |
| **Кадры экранов** | `Alebardium → UI → Screen Sheet` (в play) → `docs/lab/assets/ui-screens` |
| **Реестр экранов для кадров** | `Scripts/DevTools/UiPreviewCatalog.cs` — «id → сборка со стендовыми данными» |
| Гейт покрытия экранов кадрами | `Tests/EditMode/UI/UiScreenCatalogGateTests.cs` |
| Гейт размера контролов | `Tests/EditMode/UI/ControlSizeGateTests.cs` |
...
**Работаем в живой игре, а не на стендах** (реш. Макса 05.08.2026). Изолированный стенд отвечал на
вопрос «как это выглядит само по себе», а решение принимается по тому, как элемент выглядит В ИГРЕ —
на живом фоне, в реальном соседстве, при своей подложке. Заодно отпадает целый класс ложных выводов:
offscreen-рендер с `ppp=1` показывал обманный масштаб, а изолированный стенд — цвет без того, что
под ним лежит. Витрина `Component Gallery` по этой причине из процедуры выведена; в чеклисте сдачи
её нет.

**Каталог превью при этом ЖИВ и обязателен** (23.08.2026) — но он больше не «стенд»: сборки из
`UiPreviewCatalog` рисуются ПОВЕРХ живой панели и снимаются кадром на настоящем заднике. Он отвечает
на вопрос «как выглядит экран целиком», которого контактный лист элементов не закрывает.
...
2. **Значения — только через токены, не хардкод.**
   ...
   `rgb(...)` или `24px` в компонентах и тем более в экранах.
   *Почему:* сменить тему/масштаб = поправить один ярус токенов. Хардкод ломает это.
   *Владелец правила* — `docs/wiki/tech/10-reference/ui-design-system.md`: три яруса, кто из какого
   имеет право брать, что делать, когда роли нет. Здесь оно НЕ пересказывается — читай канон перед
   заведением новой роли или ступени. Кегли и роли текста — `ui-typography.md` там же.
...
5. **Экран сдаётся КАДРОМ, и вид собирает он сам.** (23.08.2026)
   Приёмка Макса идёт по экранам, а три самых частых промаха — реф не отработан, метрика внутри
   элемента не сходится, кусок молча пропал — не ловит ни один статический гейт.
   - Тронул экран — прогнать `Alebardium → UI → Screen Sheet` и **посмотреть кадр самой** до показа.
   - Новый экран обязан попасть в `UiPreviewCatalog`, иначе гейт красный.
   - **Стенд и игра собирают экран ОДНИМ кодом.** Сборка, запертая приватным методом роутера, —
     дефект: её нельзя ни позвать со стенда, ни снять кадром.
   - **Вид целиком внутри `Build`** — включая растяжку по слою. Данные вида передаются обязательными
     параметрами, дефолты только у колбэков: необязательный параметр вида один раз уже стоил
     пропавшей панели, и молчали обе стороны шва.

6. **Тесты — под игру и ГДД, не наоборот.**
...
- [ ] Тронул экран — снят КАДР ЭКРАНА, просмотрен самой, показан Максу
- [ ] Новый экран есть в `UiPreviewCatalog`, гейт `UiScreenCatalogGateTests` зелёный
...
- `references/ui-testing.md` — `com.unity.ui.test-framework`, PanelSimulator, смоук-тесты
  кликов.
- `references/silent-failures.md` — где UI Toolkit молчит: неизвестный токен, битый комментарий,
  проценты от ширины, подписки в `Build`, наследованные состояния. **Читать при любой правке темы
  или раскладки:** «ошибок нет» там никогда не значит «применилось».
