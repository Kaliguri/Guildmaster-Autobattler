...
```mermaid
flowchart LR
  Prep[Подготовка: расстановка, мементо] --> Battle[Автобой]
...
Единый шаблон frontmatter+структуры для новых карточек (мементо/враги/фракции/главы), чтобы
единообразие было с первого символа, а не приводилось потом. Шаблоны-эталоны уже есть:
`relics/0.2. Шаблон карточки мементо`, `enemies/0.1. Шаблон карточки противника`. Новую
