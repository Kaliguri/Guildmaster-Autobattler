...
- **Заглушки фасада НЕТ, и заводить её нельзя.** `UnityAudioService` (лог в консоль, параметры молча
  глотал) удалён 2026-07-26 в заходе по фолбэкам: он никогда не был зарегистрирован, а как «тихий
  режим» делал бы отсутствие звука неотличимым от рабочего звука. `FmodAudioService` — единственная
  реализация. Нужна подмена в тесте — мок в самом тесте, а не вторая реализация в проде (правило —
  `code-standards` §5, фолбэк только на внешний отказ). Развязка от этого не страдает: весь код и так
