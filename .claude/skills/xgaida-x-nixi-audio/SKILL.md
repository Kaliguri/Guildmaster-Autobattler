...
**Речь и музыка идут четвёртой ветвью, и она ещё НЕ реализована.** Озвучка диалогов «тоном звуков»
(voice blips) и главная тема — решения приняты 2026-07-29, кода нет: ни системы диалогов, ни категории
`voice`, ни `AudioAction.Speak`. Контракт целиком — `references/voice-and-music.md`, читать **до**
первой строчки по речи или музыке. Три вещи оттуда, которые ломаются молча:
...
- [ ] Задача по речи или музыке: `references/voice-and-music.md` прочитан; эмоция не уехала в
      локализованную строку; `SFX/Voice` вне `TimeScale`-кривой; сэмплы Celeste в игру не попали
...
- `references/pipeline-and-mix.md` — карта звука, нормализация и её числа, категории, шины,
  ...
  scripting API.
- `references/voice-and-music.md` — **речь блипами и OST**: контурная схема против метода Celeste,
  правила контура из пунктуации, где живёт эмоция, контракт категории `voice` и её готчи; закон
  «решительно, а не страшно», вокал как маркер значимости, состояния боссового трека, палитра языка,
  тариф Suno и права. **Не реализовано — это контракт, а не описание кода.**
- `references/music-tooling.md` — **роли инструментов**: Suno ведёт ВСЮ музыку (крутит Макс, Никси —
  промпт-инженер), ElevenLabs отвечает за SFX. Там же: цена и возможности ElevenLabs SFX, где генерация
  оправдана против паков, почему Suno-API не существует и чем плохи его MCP-обёртки. В конце —
  отменённый план по ElevenLabs Music (`composition_plan`), оставлен как материал.
- `references/suno-prompting.md` — **как генерировать музыку в Suno**: три ручки консистентности
  (Audio Influence держит мелодию, Persona — голос, Style — состав), два поля и Exclude, трёхслойное
  глушение вокала, вокализы конланга, диагностика→хирургия, готча лупов, заготовки промптов.
  **Ресёрч без практики: ни одной генерации ещё не сделано, часть гипотез первый прогон опровергнет.**
