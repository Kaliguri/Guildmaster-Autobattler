...
description: >-
  ...
  per-hit фидбэк (hitstop, вспышка, сплющивание, боевые цифры), VFX, death-shatter,
...
4. **Сначала смотрю САМА — раскадровкой, потом приёмка за Максом.** Приёмка его, но «эффект
   вообще делает то, что я написала» — мой вопрос, и отвечать на него его глазами нельзя.

## Раскадровка: как я проверяю визуальную правку до доклада (HARD, 07.08.2026)

**Тронул боевой VFX или анимацию — сними лист и посмотри, ПРЕЖДЕ чем говорить «готово».**

1. `Alebardium → VFX → Post FX Lab`, выбрать эффект в правом табе «Эффекты».
2. Кнопка **«Раскадровка»** — 24 кадра по всей длительности, один лист
   `Temp/PostFxLab/strip_<эффект>.png`.
3. Открыть его `Read` и разобрать: когда эффект появляется, следует ли за тем, за чем должен,
   как гаснет, нет ли разрыва между ним и его источником.

Снаружи окна (из `execute_code`) — рефлексией по приватному `ShootStrip`; стенд собирается сам,
играть его до этого не нужно.

**Почему правило есть.** 06–07.08.2026 три захода подряд «дуга идёт не так» находил Макс: у эффекта
не было других зрителей, и цикл правки шёл через его глаза. Раскадровка стоит секунды.

**Чего лист НЕ показывает.** Кадры равномерны по длительности, поэтому короткое событие внутри
длинного клипа занимает две-три клетки (дуга: пять из двадцати четырёх при клипе 1.17 с). Этого
хватает на «идёт ли след за клинком» и не хватает на разбор одного кадра — для второго пауза и
ползунок фазы.

**Максу лист показывается только `SendUserFile`** (`display: "render"`): `Read` рендерит картинку
агенту, в чат она не попадает.
...
- [ ] **Раскадровка снята и просмотрена САМОЙ** — эффект появляется когда надо, следует за чем надо
