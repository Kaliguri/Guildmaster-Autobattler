...
- **Миграция GDD-vault** — **Фаза 1 сделана** (коммит `818be538`): главы `0.x`+`1–16` →
  латинские слаги + `title` + frontmatter (`order`/`status`) + кластеры `00-meta…50-modes-ux`,
  `0.2/0.3/0.6`→`open`, `14`/`13.1`→`research/`, README→`00-meta/index` (MOC); все ссылки
  починены (0 новых битых, верифицировано). **Осталось [gated]:** Фаза 2 — карточки
  `relics/roster/enemies` на слаги; Quartz Explorer `sortFn` по `order` (untested TS, нужен
  билд сайта); 2 pre-existing мёртвые relic-ссылки в `tech/impl/05`.
